...
import cn.zying.osales.service.stocks.units.StockAdjustBillCheckUnits ;
...
public class StockAdjustBillServiceImple extends ABCommonsService implements IStockAdjustBillService {
    ...
    public SelectPage<StockAdjustBill> search(OptType optType, StockAdjustBillSearchBean searchBean, CommSearchBean commSearchBean, int... startLimit) throws SystemOptServiceException {
        return iStockAdjustBillSearchUnits.search(optType, searchBean, commSearchBean, startLimit) ;
    }
    ...
    public List<StockAdjustBill> searchList(OptType optType, StockAdjustBillSearchBean searchBean, CommSearchBean commSearchBean, int... startLimit) throws SystemOptServiceException {
        ...
        return iStockAdjustBillSearchUnits.list(optType, searchBean, commSearchBean, startLimit) ;
    ...
    public List<String> searchAdjustSubject(String adjustSubject, int... startLimit) throws SystemOptServiceException {
        String sql ;
        if (ToolsUnits.isNOtNulll(adjustSubject)) {
            sql = "select  stockAdjustBill.adjustSubject   from StockAdjustBill as  stockAdjustBill  " +

            " where stockAdjustBill.adjustSubject like '%" + adjustSubject + "%'" ;
        } else {
            sql = "select  stockAdjustBill.adjustSubject   from StockAdjustBill as  stockAdjustBill  " ;
        }
