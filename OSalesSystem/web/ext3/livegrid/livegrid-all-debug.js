/**
 * Ext.ux.grid.livegrid.GridPanel Copyright (c) 2007-2014,
...
Ext.ux.grid.livegrid.GridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	/**
	 * Removes all state events listener attached to this livegrid. The events
	 * removed are only those which are explicitely set for the livegrid's view,
	 * store and selectionmodel.
	 * 
	 * @param {Boolean}
	 *            install true to install the events, otherwise false
	 */
	installStateEvents : function(install) {
		// alert("installStateEvents "+install);
		var me = this;
	...
	/**
	 * A helper frunction that tries to automatically reload the grid given its
	 * state information, if available. If state information not available, the
	 * grid willsimple reload.
	 * 
	 * @param {Object}
	 *            state An object with the state information
	 * 
	 * @see applyState/getState
	 */
	reloadFromState : function(state) {
		// alert("reloadFromState");
		var me = this;
	...
	/**
	 * Overriden to make sure that rowIndex and buffer from the livegrid's
	 * view/store are considered when returning states.
	 * 
	 * @inheritdoc
	 */
	getState : function() {

		var state = null, me = this;
	...
	applyState : function(state) {
		// alert("ssssssssssssssssssssssssssssss applyState ");
		var me = this,
		...
		var beforeLoadCallback = Ext.createDelegate(function(store, options, conf, selections, bufferRange) {
		...
		me.store.on('beforeload', beforeLoadCallback, me, {
			single : true
		});
	...
	selectCount : 0,
	addDelSelectCount : function(count) {

		this.selectCount = this.selectCount + count;
		this.getBottomToolbar().setShowSelectCount(this.selectCount);
		this.getView().updateShowInfo();

	},

	delRowShow : function() {
		var oldselectCount = this.selectCount;
		this.selectCount = 0;
		this.getBottomToolbar().setShowSelectCount(this.selectCount);
		this.getView().updateShowInfo(this.store.getTotalCount - oldselectCount);
	},
	...
	// delSelectCount:function(count){
	// this.selectCount=this.selectCount-count;
	// this.bbar.setShowCount(this.selectCount);
	// }

});
/**
 * Ext.ux.grid.livegrid.GridView Copyright (c) 2007-2014,
 * http://www.siteartwork.de
 * 
...
Ext.ux.grid.livegrid.GridView = function(config) {
	...
	this.addEvents({
		/**
		 * @event reset Fires when the grid resets.
		 * @param {Ext.ux.grid.livegrid.GridView}
		 *            this
		 * @param {Boolean}
		 *            forceReload
		 * @param {Object}
		 *            params Additional params which get passed to the store if
		 *            forceReload was set to true
		 */
		'reset' : true,
		/**
	 ...
	 * data possible.
	 */
	this.templates.master = new Ext.Template('<div class="x-grid3" hidefocus="true"><div class="liveScroller"><div></div><div></div><div></div></div>', '<div class="x-grid3-viewport"">',

	'<div class="x-grid3-header"><div class="x-grid3-header-inner"><div class="x-grid3-header-offset" style="{ostyle}">{header}</div></div><div class="x-clear"></div></div>', '<div class="x-grid3-scroller" style="overflow-y:hidden !important;"><div class="x-grid3-body" style="{bstyle}">{body}</div><a href="#" class="x-grid3-focus" tabIndex="-1"></a></div>', "</div>", '<div class="x-grid3-resize-marker">&#160;</div>', '<div class="x-grid3-resize-proxy">&#160;</div>', "</div>");
...
Ext.extend(Ext.ux.grid.livegrid.GridView, Ext.grid.GridView, {
	 ...
	 *            othwerwise <tt>false</tt> or an object configuration with
	 *            which the data gets initialized. In this case "reset" behaves
	 *            as if forceReload was set to false
	 * @params {Object} addParams optional object of properties which get
	 *         treated as params added to the "options" argument for the load()
	 *         method if forceReload invokes a call call to this method
	 * 
	...
	reset : function(forceReload, addParams) {
		...
		if (forceReload === false || Ext.isObject(forceReload)) {
			...
			-this.liveScroller.dom.scrollTop + 1, true);
			...
			var params = Ext.apply({}, addParams), sInfo = this.ds.sortInfo;

			var pn = this.ds.paramNames, cp = {};
	...
	renderBody : function() {
		...
		var markup = this.renderRows(this.rowIndex - this.ds.bufferRange[0], ( this.rowIndex - this.ds.bufferRange[0] ) + ( this.visibleRows - 1 ));
		return this.templates.body.apply({
			rows : markup
		});
	},
	...
	doRender : function(cs, rs, ds, startRow, colCount, stripe) {
		return this._gridViewSuperclass.doRender.call(this, cs, rs, ds, startRow + this.ds.bufferRange[0], colCount, stripe);
	 ...
	 * @param {Boolean}
	 *            isLastSet tells whether the record was added at the very end
	 *            of the store, and all of the store's data was loaded, i.e.
	 *            behind the last fetched index are no more records
	 * 
	 */
	// private
	onAdd : function(ds, records, index, isLastSet) {
		...
		if (isLastSet) {
			var viewIndexFirst = firstRow + ds.bufferRange[0], viewIndexLast = lastRow + ds.bufferRange[0], html;
	...
	onBeforeLoad : function(store, options) {
		// alert(" 2 wwwwwwwwwwwwwwwwwwww onBeforeLoad "+options.forceStart+"
		// options.params.start "+options.params.start);
		// 清空
		...
		if (!options.callback) {
			options.callback = function(options, x, y) {
				// var xx=this.grid;
				if (typeof ( x.x ) == "undefined" || x.x == "0") {
					this.reset(false);
				} else {
					var xxv = {

						rowIndex : this.rowIndex,
						lastScrollPos : this.lastScrollPos,
		...
		// alert("sssssssssssss");
		var pn = store.paramNames, cp = {};
		cp[pn.start] = options.forceStart === true && options.params.start ? options.params.start : 0;
		cp[pn.limit] = this.ds.bufferSize;

		var old = store.baseParams;
		Ext.apply(old, options.params);
		apply(old, cp);
	 ...
	 */
	// private
	updateShowInfo : function(totalLength) {
		if (typeof ( totalLength ) != "undefined")
			this.ds.totalLength = totalLength;
		this.fireEvent('buffer', this, this.ds, this.rowIndex, Math.min(this.ds.totalLength, this.visibleRows - this.rowClipped), this.ds.totalLength
		// ,
		// options
		);
	},
	...
	liveBufferUpdate : function(records, options, success) {
		if (success === true) {
			...
			// alert("ssssss 1 > "+this.ds);
			// this.grid.store.loadResult(store, records, options,true,1);
			return;
		} else {
		...
		// this.grid.store.loadResult(store, options,success,1,records);
	},
		...
		if (inRange && forceReload !== true) {
			...
			if (this.ds.bufferRange[0] >= 0) {
				if (index > lastIndex) { // scrolling down
					...
					// If we have already buffered the last range we can ever
					// get
					// by the queried data repository, we don't need to buffer
					// again.
					// This basically means that a re-buffer would only occur
					// again
					...
					down = false;
					// We are scrolling up in the first buffer range we can ever
					// get
	...
	replaceLiveRows : function(cursor, forceReplace, processRows) {
		var bufferRange = this.ds.bufferRange, spill = cursor - this.lastRowIndex;
...
/**
 * Ext.ux.grid.livegrid.JsonReader Copyright (c) 2007-2014,
 * http://www.siteartwork.de
 * 
...
/**
 * Ext.ux.grid.livegrid.RowSelectionModel Copyright (c) 2007-2014,
 * http://www.siteartwork.de
 * 
...
Ext.extend(Ext.ux.grid.livegrid.RowSelectionModel, Ext.grid.RowSelectionModel, {
	...
	initEvents : function() {
		...
		var grid = this.grid, view = grid.view, store = grid.store;
	...
	getState : function() {
		...
		var me = this, selections = me.selections, selectedIds = [];
	...
	applyState : function(state) {
		...
		var me = this, ds = me.grid.store, ind;
	 ...
	 * 
	 * @param {Boolean}
	 *            fast true to reset the model's selection properties, i.e. not
	 *            triggering an event
	 */
	...
	/**
	 * Returns an object with all records currently being excluded, whereas the
	 * key is the id of the record, and it's value is set to boolean true.
	 */
...
/**
 * Ext.ux.grid.livegrid.Store Copyright (c) 2007-2014, http://www.siteartwork.de
 * 
...
Ext.ux.grid.livegrid.Store = function(config) {
	...
	'selectionsload',
	/**
	 * @event insertindexfound Fires when the store's find insertInsertIndex
	 *        method found an insert index This is for interested listeners to
	 *        be able to adjust the insert index.
	 * @param {Ext.ux.grid.livegrid.Store}
	 *            this
	 * @param {Object}
	 *            an object containing information about the insert index found
...
Ext.extend(Ext.ux.grid.livegrid.Store, Ext.data.Store, {
	...
	/**
	 * Tells whether the most recent request was successful. Note: This does not
	 * consider ongoing requests. Optimistic approach, defaults to true.
	 * 
	...
	/**
	 * Returns true if the loaded dataset represents the last dataset which can
	 * be loaded, otherwise false.
	 * 
	 * @return {Boolean}
	 */
	...
	insert : function(index, records) {
		var isLastSet = this.isLastSet(index), isAllLoaded = this.isAllLoaded();

		// hooray for haskell!
		records = [].concat(records);

		index = index >= this.bufferSize ? ( isLastSet ? index : Number.MAX_VALUE ) : index;
		...
		if (!isAllLoaded && !isLastSet && ( records.length + index >= this.bufferSize )) {
			split = true;
		...
		while (this.getCount() > this.bufferSize) {
			this.data.remove( ( isLastSet ? this.data.first() : this.data.last() ));
		}
	...
	loadRecords : function(o, options, success) {
		...
		if (!o) {
			...
			this.bufferRange = [options.params.start, Math.max(0, Math.min( ( options.params[pn.start] + options.params[pn.limit] ) - 1, o.totalRecords - 1))];
		}
		...
		// this.fireEvent('loadResult',this, options,success);

		// alert("wwww1-1");
		// function(store, options,success,loadtype){
		store_.loadResult(store_, options, success, store_.lastOptions.loadtype);
	...
	getAt : function(index) {
		...
		// index 在这里怎么获得 aaa
		var modelIndex = index - this.bufferRange[0]; // this.data.itemAt(modelIndex);
		var xx = this.data.itemAt(modelIndex);
		if (typeof ( xx ) == "undefined") {
			// alert("modelIndex "+modelIndex+" i "+index);
		}
...
/**
 * Ext.ux.grid.livegrid.CheckboxSelectionModel Copyright (c) 2007-2014,
 * http://www.siteartwork.de
 * 
...
/**
 * Ext.ux.grid.livegrid.Toolbar Copyright (c) 2007-2014,
 * http://www.siteartwork.de
 * 
...
Ext.ux.grid.livegrid.Toolbar = Ext.extend(Ext.Toolbar, {
	...
	selectCountMsg : '已选择{0}  ; ',

	isShowSelect : false,

	/**
	 * @cfg {String} bufferFailedMsg The message to display if buffering failed
	 */
	...
	/**
	 * @cfg {String} beforeloadMsg The message to display when the store is
	 *      about to load
	 */
	beforeloadMsg : 'Loading...',

	/**
	 * @cfg {String} loadFailedMsg The message to display when the store's load
	 *      operation failed
	 */
	...
	/**
	 * @type {Object} lastInfo cached version of last successfull retrieved
	 *       store information after the store was loaded
	 */
	lastInfo : null,

	selectCount : 0,

	setShowSelectCount : function(count) {
		this.selectCount = count;
	},
	...
	updateInfo : function(rowIndex, visibleRows, totalCount, context) {
		...
		if (totalCount == 0 && this.view.ds.bufferRange[0] < 0) {
			...
			if (this.lastInfo && this.lastInfo.totalLength) {
				this.displayEl.update(
						...
						String.format(this.bufferFailedMsg, String.format(this.displayMsg, rowIndex + 1, rowIndex + this.view.visibleRows, this.lastInfo.totalLength)));
				return;
			}
		...
		var msg = totalCount == 0 // selectCountMsg
		? this.emptyMsg : ( this.isShowSelect ? String.format(this.selectCountMsg, this.selectCount) : "" ) + " " + String.format(this.displayMsg, rowIndex + 1, rowIndex + visibleRows, totalCount);
	...
	onRowsInserted : function(view, start, end) {
		this.updateInfo(view.rowIndex, Math.min(view.ds.totalLength, view.visibleRows - view.rowClipped), view.ds.totalLength);
	},
...
/**
 * Ext.ux.grid.livegrid.DragZone Copyright (c) 2007-2014,
 * http://www.siteartwork.de
 * 
...
/**
 * Ext.ux.grid.livegrid.EditorGridPanel Copyright (c) 2007-2014,
 * http://www.siteartwork.de
 * 
