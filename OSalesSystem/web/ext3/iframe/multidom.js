 ...
 * ***********************************************************************************
 * License: multidom.js is offered under an MIT License. Donations are welcomed:
 * http://donate.theactivegroup.com
 */
 ...
 * @description [Designed For Ext Core and ExtJs Frameworks (using ext-base
 *              adapter only) 3.0 or higher ONLY] The multidom library extends
 *              (overloads) Ext Core DOM methods and functions to provide
 *              document-targeted access to the documents loaded in external
 *              (FRAME/IFRAME) documents.
 *              <p>
 *              It maintains seperate DOM Element caches (and more) for each
 *              document instance encountered by the framework, permitting safe
 *              access to DOM Elements across document instances that may share
 *              the same Element id or name. In essence, multidom extends the
 *              functionality provided by Ext Core into any child document
 *              without having to load the Core library into the frame's global
 *              context.
 *              <h3>Custom Element classes.</h3>
 *              The Ext.get method is enhanced to support resolution of the
 *              custom Ext.Element implementations. (The ux.ManagedIFrame 2
 *              Element class is an example of such a class.)
 *              <p>
 *              For example: If you were retrieving the Ext.Element instance for
 *              an IFRAME and the class Ext.Element.IFRAME were defined:
 * 
 * <pre><code>Ext.get('myFrame')</pre></code> would return an instance of Ext.Element.IFRAME for
 *              'myFrame' if it were found.
 *              @example // return the Ext.Element with an id 'someDiv' located in external document hosted by 'iframe'
 ...
 * 
 * Likewise, you can retrieve the raw Element of another document with: var el =
 * Ext.getDom('myDiv', iframe.getFrameDocument());
 */

( function() {

	/*
	 * Ext.Element and Ext.lib.DOM enhancements. Primarily provides the ability
	 * to interact with any document context (not just the one Ext was loaded
	 * into).
	 */
	var El = Ext.Element, ElFrame, ELD = Ext.lib.Dom, A = Ext.lib.Anim, Evm = Ext.EventManager, E = Ext.lib.Event, DOC = document, emptyFn = function() {
	}, OP = Object.prototype, OPString = OP.toString, bodyTag = /^body/i, HTMLDoc = '[object HTMLDocument]';
	...
	/**
	 * @private Resolve the Element cache for a given element/window/document
	 *          context.
	 */
	var resolveCache = ELD.resolveDocumentCache = function(el, cacheId) {

		/**
		 * MUST re-assert Ext.elCache !! because of privately scoped references
		 * to Ext.elCache in the framework itself.
		 */
		Ext._documents[Ext.id(document, '_doc')] = Ext.elCache;

		var doc = GETDOC(el), c = Ext.isDocument(doc) ? Ext.id(doc) : cacheId, cache = Ext._documents[c] || null;
	...
	Ext.removeNode = function(n) {
		var dom = n ? n.dom || n : null, el, elc, elCache = resolveCache(dom), parent;
	...
	Ext.applyIf(Ext, {
		 ...
		 * 
		 * @param {HTMLELement}
		 *            el The DOM Element to test
		 * @param {Boolean}
		 *            testOrigin (optional) True to test "same-origin" access
		 * 
		 */
		...
		/**
		 * Determine whether a specified DOMEvent is supported by a given
		 ...
		 *            passed eventName is used, or DIV as default.
	...
	/**
	 * @private Determine Ext.Element[tagName] or Ext.Element (default)
	 */
	...
	Ext.apply(Ext, {
		/*
		 * Overload Ext.get to permit Ext.Element access to other document
		 * objects This implementation maintains safe element caches for each
		 * document queried.
		 * 
		 */
			...
			if (typeof el == "string") { // element id
				...
				if (el.dom) {
					...
					( cache[el.id] || ( cache[el.id] = {
						data : {},
						events : {}
					} ) ).el = el; // in case it was created directly with
		...
		getDoc : Ext.overload([Ext.getDoc, function(doc) {
	...
	/*
	 * Add new Visibility Mode to element (sets height and width to 0px instead
	 * of display:none )
	 */
	El.OFFSETS = 3;
	El.ASCLASS = 4;
	...
	var propCache = {}, camelRe = /(-[a-z])/gi, camelFn = function(m, a) {
		return a.charAt(1).toUpperCase();
	}, opacityRe = /alpha\(opacity=(.*)\)/i, trimRe = /^\s+|\s+$/g, marginRightRe = /marginRight/, propFloat = Ext.isIE ? 'styleFloat' : 'cssFloat', view = DOC.defaultView, VISMODE = 'visibilityMode', ASCLASS = "asclass", ORIGINALDISPLAY = 'originalDisplay', PADDING = "padding", MARGIN = "margin", BORDER = "border", LEFT = "-left", RIGHT = "-right", TOP = "-top", BOTTOM = "-bottom", WIDTH = "-width", MATH = Math, OPACITY = "opacity", VISIBILITY = "visibility", DISPLAY = "display", OFFSETS = "offsets", NOSIZE = 'nosize', ASCLASS = "asclass", HIDDEN = "hidden", NONE = "none", ISVISIBLE = 'isVisible', ISCLIPPED = 'isClipped', OVERFLOW = 'overflow', OVERFLOWX = 'overflow-x', OVERFLOWY = 'overflow-y', ORIGINALCLIP = 'originalClip', XMASKED = "x-masked", XMASKEDRELATIVE = "x-masked-relative",
	...
	borders = {
		...
		t : MARGIN + TOP,
		b : MARGIN + BOTTOM
	}, data = El.data, GETDOM = Ext.getDom, GET = Ext.get, DH = Ext.DomHelper, propRe = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/, CSS = Ext.util.CSS, // Not
																																																					// available
																																																					// in
																																																					// Ext
																																																					// Core.
	...
	El.addMethods({
		 ...
		 * 
		 * @param {Boolean}
		 *            cleanse (optional) Perform a cleanse of immediate
		 *            childNodes as well.
		 * @param {Boolean}
		 *            deep (optional) Perform a deep cleanse of all nested
		 *            childNodes as well.
		 */
		 ...
		 *            el
		 * @param {Document}
		 *            doc (optional) specific document context for the Element
		 *            search
		 ...
		 * 
		 * @param {Mixed}
		 *            el The element before which this element will be inserted
		 * @param {Document}
		 *            doc (optional) specific document context for the Element
		 *            search
		 * @return {Ext.Element} this
		...
		/**
		 * Inserts (or creates) an element (or DomHelper config) as the first
		 * child of this element
		 * 
		 * @param {Mixed/Object}
		 *            el The id or element to insert or a DomHelper config to
		 *            create and insert
		 * @param {Document}
		 *            doc (optional) specific document context for the Element
		 *            search
		 ...
		 * 
		 * @param {Mixed/Object}
		 *            el The new element or a DomHelper config of an element to
		 *            create
		 * @param {Document}
		 *            doc (optional) specific document context for the Element
		 *            search
		 ...
		 * 
		 * @param {String}
		 *            where Where to insert the html in relation to this element -
		 *            beforeBegin, afterBegin, beforeEnd, afterEnd.
		 * @param {String}
		 *            html The HTML fragment
		 * @param {Boolean}
		 *            returnEl (optional) True to return an Ext.Element
		 *            (defaults to false)
		 * @return {HTMLElement/Ext.Element} The inserted node (or nearest
		 *         related if more than 1 inserted)
		 */
		...
		isVisible : function(deep) {
			var me = this, dom = me.dom, p = dom.parentNode, visible = data(dom, ISVISIBLE); // use
																								// the
																								// cached
																								// value
																								// if
			...
			if (typeof visible != 'boolean') {
				...
				visible = !me.hasClass(me.visibilityCls || El.visibilityCls) && !me.isStyle(VISIBILITY, HIDDEN) && !me.isStyle(DISPLAY, NONE);
		...
		/**
		 * Sets the visibility of the element (see details). If the
		 * visibilityMode is set to Element.DISPLAY, it will use the display
		 * property to hide the element, otherwise it uses visibility. The
		 * default is to hide and show using the visibility property.
		 * 
		 * @param {Boolean}
		 *            visible Whether the element is visible
		 * @param {Boolean/Object}
		 *            animate (optional) True for the default animation, or a
		 *            standard Element animation config object, or one of four
		 *            possible hideMode strings: 'display, visibility, offsets,
		 *            asclass'
		 * @return {Ext.Element} this
		 */
		setVisible : function(visible, animate) {
			var me = this, dom = me.dom, visMode = getVisMode(dom);
		 ...
		 * 
		 * @param {Mixed}
		 *            value Boolean value to display the element using its
		 *            default display, or a string to set the display directly.
		 * @return {Ext.Element} this
		 */
		setDisplayed : function(value) {
			var dom = this.dom, visMode = getVisMode(dom);
		 ...
		 * 
		 * @param {String}
		 *            display (optional) What to set display to when visible
		 * @return {Ext.Element} this
		 */
		...
		scrollIntoView : function(container, hscroll) {
			var d = this.getDocument(), c = Ext.getDom(container, null, d) || Ext.getBody(d).dom, el = this.dom, o = this.getOffsetsTo(c), l = o[0] + c.scrollLeft, t = o[1] + c.scrollTop, b = t + el.offsetHeight, r = l + el.offsetWidth, ch = c.clientHeight, ct = parseInt(c.scrollTop, 10), cl = parseInt(c.scrollLeft, 10), cb = ct + ch, cr = cl + c.clientWidth;
		 ...
		 * 
		 * @return {Object} An object containing the scroll position in the
		 *         format {left: (scrollLeft), top: (scrollTop)}
		 */
		getScroll : function() {
			var d = this.dom, doc = this.getDocument(), body = doc.body, docElement = doc.documentElement, l, t, ret;
		 ...
		 * @return {String} The current value of the style property for this
		 *         element.
		 */
		getStyle : function() {
			var getStyle = view && view.getComputedStyle ? function GS(prop) {
				var el = !this._isDoc ? this.dom : null, v, cs, out, display, wk = Ext.isWebKit, display, style;
				...
				if (wk) {
					if (out && marginRightRe.test(prop) && style.position != 'absolute' && out != '0px') {
				...
				var el = !this._isDoc ? this.dom : null, m, cs, style;
				if (!el || !el.style)
					return null;
				style = el.style;
				if (prop == OPACITY) {
		 ...
		 * 
		 * @param {String/Object}
		 *            property The style property to be set, or an object of
		 *            multiple styles.
		 * @param {String}
		 *            value (optional) The value to apply to the given property,
		 *            or null if an object was passed.
		 * @return {Ext.Element} this
		 */
		 ...
		 * 
		 * @param {Mixed}
		 *            centerIn (optional) The element in which to center the
		 *            element.
		 */
		...
		/**
		 * Puts a mask over this element to disable user interaction. Requires
		 * core.css. This method can only be applied to elements which accept
		 * child nodes.
		 * 
		 * @param {String}
		...
		mask : function(msg, msgCls) {
			...
			if (Ext.isIE && ! ( Ext.isIE7 && Ext.isStrict ) && me.getStyle('height') == 'auto') { // ie
																									// will
																									// not
																									// expand
																									// full
		...
		/**
		 * Gets the x,y coordinates specified by the anchor position on the
		 * element.
		 * 
		 * @param {String}
		 *            anchor (optional) The specified anchor position (defaults
		 *            to "c"). See {@link #alignTo} for details on supported
		 *            anchor positions.
		 * @param {Boolean}
		 *            local (optional) True to get the local (element
		 *            top/left-relative) anchor position instead of page
		 *            coordinates
		 * @param {Object}
		 *            size (optional) An object containing the size to use for
		 *            calculating anchor position {width: (target width),
		 *            height: (target height)} (defaults to the element's
		 *            current size)
		 * @return {Array} [x, y] An array containing the element's x and y
		 *         coordinates
		 */
		getAnchorXY : function(anchor, local, s) {
			...
			var me = this, doc = this.getDocument(), vp = me.dom == doc.body || me.dom == doc, w = s.width || vp ? ELD.getViewWidth(false, doc) : me.getWidth(), h = s.height || vp ? ELD.getViewHeight(false, doc) : me.getHeight(), xy, r = Math.round, o = me.getXY(), scroll = me.getScroll(), extraX = vp ? scroll.left : !local ? o[0] : 0, extraY = vp ? scroll.top : !local ? o[1] : 0, hash = {
				c : [r(w * .5), r(h * .5)],
				t : [r(w * .5), 0],
				l : [0, r(h * .5)],
				r : [w, r(h * .5)],
		...
		/**
		 * Anchors an element to another element and realigns it when the window
		 * is resized.
		 * 
		 * @param {Mixed}
		 ...
		 *            monitorScroll (optional) True to monitor body scroll and
		 *            reposition. If this parameter is a number, it is used as
		 *            the buffer delay (defaults to 50ms).
		 * @param {Function}
		...
		getAlignToXY : function(el, p, o) {
			...
			var me = this, d = me.dom, a1, a2, x, y,
			// constrain the aligned el to viewport if necessary
			w, h, r, dw = ELD.getViewWidth(false, doc) - 10, // 10px of
																// margin for ie
			dh = ELD.getViewHeight(false, doc) - 10, // 10px of margin for ie
			p1y, p1x, p2y, p2x, swapY, swapX, docElement = doc.documentElement, docBody = doc.body, scrollX = ( docElement.scrollLeft || docBody.scrollLeft || 0 ) + 5, scrollY = ( docElement.scrollTop || docBody.scrollTop || 0 ) + 5, c = false, // constrain
																																																														// to
																																																														// viewport
			p1 = "", p2 = "", m = p.match(/^([a-z]+)-([a-z]+)(\?)?$/);
		...
		getConstrainToXY : function(el, local, offsets, proposedXY) {
			...
			return function(el, local, offsets, proposedXY) {
				...
				var vr = vx + vw, vb = vy + vh, xy = proposedXY || ( !local ? this.getXY() : [this.getLeft(true), this.getTop(true)] );
		 ...
		 * @param {Number/Mixed}
		 *            maxDepth (optional) The max depth to search as a number or
		 *            element (defaults to 50 || document.body)
		 * @param {Boolean}
		 *            returnEl (optional) True to return a Ext.Element object
		 *            instead of DOM node
		 * @return {HTMLElement} The matching DOM node (or null if no match was
		 *         found)
		 */
		findParent : function(simpleSelector, maxDepth, returnEl) {
			var p = this.dom, D = this.getDocument(), b = D.body, depth = 0, stopEl;
		 ...
		 * use <tt>{@link #unclip}</tt> to remove
		 * 
		 * @return {Ext.Element} this
		 */
		clip : function() {
		...
		getViewSize : function() {
			var doc = this.getDocument(), d = this.dom, isDoc = ( d == doc || d == doc.body );
		...
		/**
		 * <p>
		 * Returns the dimensions of the element available to lay content out
		 * in.
		 * <p>
		 * 
		 * getStyleSize utilizes prefers style sizing if present, otherwise it
		 * chooses the larger of offsetHeight/clientHeight and
		 * offsetWidth/clientWidth. To obtain the size excluding scrollbars, use
		 * getViewSize
		 * 
		 * Sizing of the document body is handled at the adapter level which
		 * handles special cases for IE and strict modes, etc.
		 */

		getStyleSize : function() {
			var me = this, w, h, doc = this.getDocument(), d = this.dom, isDoc = ( d == doc || d == doc.body ), s = d.style;
	...
	Ext.apply(ELD, {
		...
		getDocument : function(el, accessTest) {
			var dom = null;
			try {
				dom = Ext.getDom(el, null, null); // will fail if El.dom is
													// non "same-origin"
													// document
			} catch (ex) {
			}
			...
			return dom ? dom.ownerDocument || // Element
			dom.document // Window
			: null;
		},
		...
		getViewWidth : Ext.overload([ELD.getViewWidth || function(full) {
		}, function() {
		...
		getViewHeight : Ext.overload([ELD.getViewHeight || function(full) {
		}, function() {
		...
		getDocumentHeight : Ext.overload([ELD.getDocumentHeight || emptyFn, function(doc) {
		...
		getDocumentWidth : Ext.overload([ELD.getDocumentWidth || emptyFn, function(doc) {
			if (doc = this.getDocument(doc)) {
		...
		getViewportHeight : Ext.overload([ELD.getViewportHeight || emptyFn, function(doc) {
			if (doc = this.getDocument(doc)) {
				if (Ext.isIE) {
		...
		getViewportWidth : Ext.overload([ELD.getViewportWidth || emptyFn, function(doc) {
			if (doc = this.getDocument(doc)) {
		...
		getXY : Ext.overload([ELD.getXY || emptyFn, function(el, doc) {
			if (typeof el == 'string') {
				el = Ext.getDom(el, null, doc);
				var D = this.getDocument(el), bd = D ? ( D.body || D.documentElement ) : null;
	...
	var GETDOC = ELD.getDocument, flies = El._flyweights;

	/**
	 * @private Add Ext.fly support for targeted document contexts
	 */
	...
	function addListener(el, ename, fn, task, wrap, scope) {
		...
		var id = Ext.id(el), cache = resolveCache(el);
		...
		// this is a workaround for jQuery and should somehow be removed from
		// Ext Core in the future
		...
		if (ename == "mousedown" && DOC == el) { // fix stopped mousedowns on
													// the document
	...
	function listen(element, ename, opt, fn, scope) {
		var o = !Ext.isObject(opt) ? {} : opt, el = Ext.getDom(element), task;
	...
	Ext.apply(Evm, {
		 ...
		 * {@link #un} is equivalent. Typically you will use
		 * {@link Ext.Element#removeListener} directly on an Element in favor of
		 * calling this version.
		 * 
		 * @param {String/HTMLElement}
		 *            el The id or html element from which to remove the
		 *            listener.
		 * @param {String}
		 *            eventName The name of the event.
		 * @param {Function}
		 *            fn The handler function to remove. <b>This must be a
		 *            reference to the function passed into the
		 *            {@link #addListener} call.</b>
		 * @param {Object}
		 *            scope If a scope (<b><code>this</code></b> reference)
		 *            was specified when the listener was added, then this must
		 *            refer to the same object.
		 */
		removeListener : Evm.un = function(element, eventName, fn, scope) {
			var el = Ext.getDom(element);
			el && Ext.get(el);
			var elCache = el ? resolveCache(el) : {}, f = el && ( ( elCache[el.id] || {
				events : {}
			} ).events )[eventName] || [], wrap, i, l, k, len, fnc, evs;

			for (i = 0, len = f.length; i < len; i++) {
				/*
				 * 0 = Original Function, 1 = Event Manager Wrapped Function, 2 =
				 * Scope, 3 = Adapter Wrapped Function, 4 = Buffered Task
				 */
				if (Ext.isArray(fnc = f[i]) && fnc[0] == fn && ( !scope || fnc[2] == scope )) {
					...
					if (wrap && eventName == "mousedown" && DOC == el) { // fix
																			// stopped
																			// mousedowns
																			// on
																			// the
		...
		/**
		 * Removes all event handers from an element. Typically you will use
		 * {@link Ext.Element#removeAllListeners} directly on an Element in
		 * favor of calling this version.
		 * 
		 * @param {String/HTMLElement}
		 *            el The id or html element from which to remove all event
		 *            handlers.
		 */
		removeAll : function(el) {
			...
			var id = el.id, elCache = resolveCache(el) || {}, es = elCache[id] || {}, ev = es.events || {}, f, i, len, ename, fn, k, wrap;

			for (ename in ev) {
				if (ev.hasOwnProperty(ename)) {
					f = ev[ename];
					/*
					 * 0 = Original Function, 1 = Event Manager Wrapped
					 * Function, 2 = Scope, 3 = Adapter Wrapped Function, 4 =
					 * Buffered Task
					 */
					for (i = 0, len = f.length; i < len; i++) {
						fn = f[i];
						...
						// jQuery workaround that should be removed from Ext
						// Core
		...
		getListeners : function(el, eventName) {
			...
			var id = ( Ext.get(el) || {} ).id, elCache = resolveCache(el), es = ( elCache[id] || {} ).events || {};
		...
		purgeElement : function(el, recurse, eventName) {
			el = Ext.getDom(el);
			var id = Ext.id(el), elCache = resolveCache(el), es = ( elCache[id] || {} ).events || {}, i, f, len;
			if (eventName) {
