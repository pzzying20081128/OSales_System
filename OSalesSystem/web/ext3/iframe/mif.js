 ...
 * ***********************************************************************************
 * 
 * @version 2.1.4 [For Ext 3.1.1 or higher only]
 * 
 * License: ux.ManagedIFrame, ux.ManagedIFrame.Panel, ux.ManagedIFrame.Portlet,
 * ux.ManagedIFrame.Window are licensed under the terms of the Open Source GPL
 * 3.0 license: http://www.gnu.org/licenses/gpl.html
 * 
...
( function() {
	 ...
	 *         href="http://extjs.com/forum/member.php?u=8730">hendricd</a>
	 * @donate <a target="tag_donate" href="http://donate.theactivegroup.com"><img
	 *         border="0"
	 *         src="http://www.paypal.com/en_US/i/btn/x-click-butcc-donate.gif"
	 *         border="0" alt="Make a donation to support ongoing development"></a>
	 * @copyright 2007-2010, Active Group, Inc. All rights reserved.
	 * @constructor Create a new Ext.ux.ManagedIFrame.Element directly.
	 * @param {String/HTMLElement}
	 *            element
	 * @param {Boolean}
	 *            forceNew (optional) By default the constructor checks to see
	 *            if there is already an instance of this element in the cache
	 *            and if there is it returns the same instance. This will skip
	 *            that check (useful for extending this class).
	 * @param {DocumentElement}
	 *            (optional) Document context uses to resolve an Element search
	 *            by its id.
	 */
	...
	Ext.ux.ManagedIFrame.Element = Ext.extend(Ext.Element, {
		...
		constructor : function(element, forceNew, doc) {
			var d = doc || document, elCache = ELD.resolveDocumentCache(d), dom = Ext.getDom(element, false, d);

			if (!dom || ! ( /^(iframe|frame)/i ).test(dom.tagName)) { // invalid
																		// id/element
				return null;
			}
			var id = Ext.id(dom);
			...
			if (!this._observable) {
				 ...
				 * @param {Ext.ux.MIF.Element}
				 *            this
				 */
				'documentloaded',

				/**
				 * Fires ONLY when an iFrame's Document(DOM) has reach a state
				 * where the DOM may be manipulated ('same origin' policy) Note:
				 * This event is only available when overwriting the iframe
				 * document using the update or load methods and "same-origin"
				 * documents. Returning false from the eventHandler stops
				 * further event (documentloaded) processing.
				 * 
				 * @event domready
				 * @param {Ext.ux.MIF.Element}
				 *            this
				...
				/**
				 * Fires when the frame's window is resized. This event, when
				 * raised from a "same-origin" frame, will send current
				 * height/width reports with the event.
				 * 
				 * @event resize
				 * @param {Ext.ux.MIF.Element}
				 *            this.iframe
				 * @param {Object}
				 *            documentSize A height/width object signifying the
				 *            new document size
				 * @param {Object}
				 *            viewPortSize A height/width object signifying the
				 *            size of the frame's viewport
				 * @param {Object}
				 *            viewSize A height/width object signifying the size
				 *            of the frame's view
				 */
				'resize',
				 ...
				 * window.sendMessage method of the embedded Iframe.window
				 * object
				 * 
				 * @event message
				 * @param {Ext.ux.MIF}
				 *            this.iframe
				 * @param {object}
				 *            message (members: type: {string} literal
				 *            "message", data {Mixed} [the message payload],
				 *            domain [the document domain from which the message
				 *            originated ], uri {string} the document URI of the
				 *            message sender source (Object) the window context
				 *            of the message sender tag {string} optional
				 *            reference tag sent by the message sender
				 *            <p>
				 *            Alternate event handler syntax for message:tag
				 *            filtering Fires upon receipt of a message
				 *            generated by window.sendMessage method which
				 *            includes a specific tag value of the embedded
				 ...
				 *            this
				 * @param {Ext.Event}
				 *            Note: This event is only available when
				 *            overwriting the iframe document using the update
				 *            method and to pages retrieved from a "same
				 *            domain". Returning false from the eventHandler
				 *            [MAY] NOT cancel the event, as this event is NOT
				 ...
				 * update method and to pages retrieved from a "same domain".
				 * Returning false from the eventHandler [MAY] NOT cancel the
				 * event, as this event is NOT ALWAYS cancellable in all
				 * browsers.
				 * 
				 * @event focus
				 * @param {Ext.ux.MIF.Element}
				 ...
				 * retrieved from a "same-origin" domain. Note: Opera does not
				 * raise this event.
				 * 
				 * @event unload * Fires when(if) the frames window object
				 ...
				 * Note: This event is only available when overwriting the
				 * iframe document using the update method and to pages
				 * retrieved from a "same-origin" domain. To prevent numerous
				 * scroll events from being raised use the buffer listener
				...
				/**
				 * Fires when the iFrame has been reset to a neutral domain
			...
			this.on(Ext.isIE ? 'readystatechange' : 'load', this.loadHandler, this,
			/**
			 * Opera still fires LOAD events for images within the FRAME as
			 * well, so we'll buffer hopefully catching one of the later events
			 */
			Ext.isOpera ? {
		...
		/**
		 * @private Removes the MIFElement interface from the FRAME Element. It
		 *          does NOT remove the managed FRAME from the DOM. Use the
		 *          {@link Ext.#ux.ManagedIFrame.Element-remove} method to
		 *          perfom both functions.
		 */
		destructor : function() {
		 ...
		 * 
		 * @param {Boolean}
		 *            forceReclean (optional) By default the element keeps track
		 *            if it has been cleansed already so you can call this over
		 *            and over. However, if you update the element and need to
		 *            force a reclean, you can pass true.
		 * @param {Boolean}
		 *            deep (optional) Perform a deep cleanse of all childNodes
		 *            as well.
		 */
		 ...
		 * the Ext.util.CSS singleton to manipulate the style sheets of the
		 * frame's embedded document.
		 * 
		 * @property
		 * @type Ext.util.CSS
		 */
		...
		/**
		 * @cfg {Number} operaLoadBuffer Listener buffer time (in milliseconds)
		 *      to buffer Opera's errant load events (fired for inline images as
		 *      well) for IFRAMES.
		 */
		...
		/**
		 * @cfg {Integer} domReadyRetries Maximum number of domready event
		 *      detection retries for IE. IE does not provide a native DOM event
		 *      to signal when the frames DOM may be manipulated, so a polling
		 *      process is used to determine when the documents BODY is
		 *      available.
		 *      <p>
		 *      Certain documents may not contain a BODY tag: eg. MHT(rfc/822),
		 *      XML, or other non-HTML content. Detection polling will stop
		 *      after this number of 2ms retries or when the documentloaded
		 *      event is raised.
		 *      </p>
		...
		/**
		 * True to set focus on the frame Window as soon as its document reports
		 * loaded.
		 * <p>
		 * (Many external sites use IE's document.createRange to create DOM
		 * elements, but to be successful, IE requires that the FRAME have focus
		 * before such methods are called)
		 * </p>
		 * 
		 * @cfg focusOnLoad
		 * @default true if IE
		...
		/**
		 * Toggles raising of events for URL actions that the Component did not
		 * initiate.
		 * 
		 * @cfg {Boolean} eventsFollowFrameLinks set true to propogate domready
		 *      and documentloaded events anytime the IFRAME's URL changes
		 * @default true
		 */
		...
		/**
		 * Return the ownerDocument property of the IFRAME Element. (Note: This
		 * is not the document context of the FRAME's loaded document. See the
		 * getFrameDocument method for that.)
		 */
		 ...
		 * 
		 * @param {Object}
		 *            submitCfg A config object containing any of the following
		 *            options:
		 * 
		 * <pre><code>
		...
		submitAsTarget : function(submitCfg) {
			var opt = submitCfg || {}, D = this.getDocument(), form = Ext.getDom(opt.form ? opt.form.form || opt.form : null, false, D) || Ext.DomHelper.append(D.body, {
				tag : 'form',
				...
				action : form.action || ''
			}, encoding = opt.encoding || form.encoding, method = opt.method || form.method || 'POST';
	...
	// slight delay for masking
		...
		/**
		 * @cfg {String} resetUrl Frame document reset string for use with the
		 *      {@link #Ext.ux.ManagedIFrame.Element-reset} method. Defaults:
		 *      <p>
		 *      For IE on SSL domains - the current value of Ext.SSL_SECURE_URL
		 *      <p>
		 ...
		 *            url (Optional) A string or reference to a Function that
		 *            returns a URI string when called
		 * @param {Boolean}
		 *            discardUrl (Optional) If not passed as <tt>false</tt>
		...
		/**
		 * Sets the embedded Iframe location using its replace method
		 * (precluding a history update). Note: invoke the function with no
		 * arguments to refresh the iframe based on the current src value.
		 * 
		 * @param {String/Function}
		 *            url (Optional) A string or reference to a Function that
		 *            returns a URI string when called
		...
		/**
		 * @private Regular Expression filter pattern for script tag removal.
		 * @cfg {regexp} scriptRE script removal RegeXp Default: "/(?:<script.*?>)((\n|\r|.)*?)(?:<\/script>)/gi"
		 */
		scriptRE : /(?:<script.*?>)((\n|\r|.)*?)(?:<\/script>)/gi,
		 ...
		 * @param {Boolean}
		 *            loadScripts (optional) true to also render and process
		 *            embedded scripts
		 * @param {Function}
		 *            callback (Optional) A callback function invoked when the
		 *            frame document has been written and fully loaded.
		 * @param {Object}
		 *            scope (Optional) scope by which the callback function is
		 *            invoked.
		 */
		...
		/**
		 * Executes a Midas command on the current document, current selection,
		 * or the given range.
		 * 
		 * @param {String}
		 *            command The command string to execute in the frame's
		 *            document context.
		 * @param {Booloean}
		 *            userInterface (optional) True to enable user interface (if
		 *            supported by the command)
		 * @param {Mixed}
		 *            value (optional)
		 * @param {Boolean}
		 *            validate If true, the command is validated to ensure it's
		 ...
		 *            active True (or "on"), to enable designMode
		 * 
		 */
		 ...
		 * @param {Boolean}
		 *            unique (optional) True to create a unique Ext.Element for
		 *            each child (defaults to false, which creates a single
		 *            shared flyweight object)
		 * @return {Ext.CompositeElement/Ext.CompositeElementLite} The composite
		 *         element
		 */
		 ...
		 *            node The node id or node Element to remove
		 */
		...
		/**
		 * Attempt to retrieve the frames current URI via frame's document
		 * object
		 * 
		 * @return {string} The frame document's current URI or the last know
		 *         URI if permission was denied.
		 */
		...
		/**
		 * Scrolls a frame document's child element into view within the passed
		 * container.
		 * 
		 * @param {String}
		 *            child The id of the element to scroll into view.
		 * @param {Mixed}
		 *            container (optional) The container element to scroll
		 *            (defaults to the frame's document.body). Should be a
		 *            string (id), dom node, or Ext.Element.
		 * @param {Boolean}
		 *            hscroll (optional) False to disable horizontal scroll
		 *            (defaults to true)
		 * @return {Ext.ux.ManagedIFrame.Element} this
		 */
		...
		/**
		 * Returns the general DOM modification capability (same-origin status)
		 * of the frame.
		 * 
		 * @return {Boolean} accessible If True, the frame's inner DOM can be
		 *         manipulated, queried, and Event Listeners set.
		 */
		domWritable : function() {
			return !!Ext.isDocument(this.getFrameDocument(), true) // test
																	// access
					&& !!this._windowContext;
		},
		 ...
		 * 
		 * @param {String}
		 *            block A valid ('eval'able) script source block.
		 * @param {Boolean}
		 *            useDOM if true, inserts the function into a dynamic script
		 *            tag, false does a simple eval on the function definition.
		 *            (useful for debugging)
		 *            <p>
		 *            Note: will only work after a successful iframe.(Updater)
		 *            update or after same-domain document has been hooked,
		 ...
		 * 
		 * @param {String}
		 *            block A valid (executable) script source block.
		 * @param {object}
		 *            attributes Additional Script tag attributes to apply to
		 *            the script Element (for other language specs [vbscript,
		 *            Javascript] etc.)
		 *            <p>
		 *            Note: writeScript will only work after a successful
		 *            iframe.(Updater) update or after same-domain document has
		 ...
		 * 
		 * @param {String/Object}
		 *            fn Name of the function or function map object:
		 *            {name:'encodeHTML',fn:Ext.util.Format.htmlEncode}
		 * @param {Boolean}
		 *            useDOM if true, inserts the fn into a dynamic script tag,
		 *            false does a simple eval on the function definition
		 * @param {Boolean}
		 *            invokeIt if true, the function specified is also executed
		 *            in the Window context of the frame. Function arguments are
		 *            not supported.
		 *            @example
		 * 
		 * <pre><code> var trim = function(s){ return s.replace(/^\s+|\s+$/g,''); }; 
		...
		loadFunction : function(fn, useDOM, invokeIt) {
			...
			name && fnSrc && this.execScript(name + '=' + fnSrc, useDOM); // fn.toString
																			// coercion
			invokeIt && this.execScript(name + '()'); // no args only
		},

		/**
		 * @private Evaluate the Iframes readyState/load event to determine its
		 *          'load' state, and raise the 'domready/documentloaded' event
		 *          when applicable.
		 */
		...
		/**
		 * @private Poll the Iframes document structure to determine DOM ready
		...
		checkDOM : function(win) {
			...
			var n = 0, frame = this, domReady = false, b, l, d, max = this.domReadyRetries || 2500, // default
																									// max
																									// 5
																									// seconds
			polling = false, startLocation = ( this.getFrameDocument() || {
				location : {}
			} ).location.href;
	( function() { // DOM polling for IE and others
		...
		/**
		 * @private override Removes an event handler from this element.
		 */
		 ...
		 * 
		 * @param {String}
		 *            msg Mask text to display during the mask operation,
		 *            defaults to previous defined loadMask config value.
		 * @param {String}
		 *            msgCls The CSS class to apply to the loading message
		 *            element (defaults to "x-mask-loading")
		 * @param {String}
		 *            maskCls The CSS class to apply to the mask element
		 */
		 ...
		 * 
		 * @param {Boolean}
		 *            forced True to hide the mask regardless of document
		 *            ready/loaded state.
		 */
		...
		/**
		 * Puts a mask over the FRAME to disable user interaction. Requires
		 * core.css.
		 * 
		 * @param {String}
		 ...
		 * @param {String}
		 *            maskCls (optional) A css class to apply to the mask
		 *            element
		 * @param {String/Element}
		 *            maskEl (optional) A targeted Element (parent of the
		 *            IFRAME) to use the masking agent
		...
		mask : function(msg, msgCls, maskCls, maskEl) {
			...
			if (Ext.isIE && ! ( Ext.isIE7 && Ext.isStrict ) && this.getStyle('height') == 'auto') { // ie
																									// will
																									// not
																									// expand
																									// full
		...
		/**
		 * Creates an (frontal) transparent shim agent for the frame. Used
		 * primarily for masking the frame during drag operations.
		 * 
		 * @return {Ext.Element} The new shim element.
		 * @param {String}
		 *            imgUrl Optional Url of image source to use during shimming
		 *            (defaults to Ext.BLANK_IMAGE_URL).
		 * @param {String}
		 *            shimCls Optional CSS style selector for the shimming
		 *            agent. (defaults to 'ux-mif-shim' ).
		 * @return (HTMLElement} the shim element
		 */
		createFrameShim : function(imgUrl, shimCls) {
			this.shimCls = shimCls || this.shimCls || 'ux-mif-shim';
			this.frameShim || ( this.frameShim = this.next('.' + this.shimCls) || // already
																					// there
																					// ?
		...
		/**
		 * Toggles visibility of the (frontal) transparent shim agent for the
		 * frame. Used primarily for masking the frame during drag operations.
		 * 
		 * @param {Boolean}
		 *            show Optional True to activate the shim, false to hide the
		 *            shim agent.
		 */
		...
		/**
		 * @private Frame document event proxy
		 */
		...
		/**
		 * dispatch a message to the embedded frame-window context (same-origin
		 * frames only)
		 * 
		 * @name sendMessage
		 * @param {Mixed}
		 *            message The message payload. The payload can be any
		 *            supported JS type.
		 * @param {String}
		 *            tag Optional reference tag
		 * @param {String}
		 *            origin Optional domain designation of the sender (defaults
		 *            to document.domain).
		 */
		...
		/**
		 * Dispatch a cross-document message (per HTML5 specification) if the
		 * browser supports it natively.
		 * 
		 * @name postMessage
		 * @param {String}
		 *            message Required message payload (String only)
		 * @param {String}
		 *            origin (Optional) Site designation of the sender (defaults
		 *            to the current site in the form: http://site.example.com ).
		 *            <p>
		 *            Notes: on IE8, this action is synchronous.<br/> Messaging
		 *            support requires that the optional messaging driver source
	...
	Ext.override(ElFrame, {
		 ...
		 * @param {Object}
		 *            scope (optional) The scope (this element) of the handler
		 *            function
		 * @param {Object}
		...
		/**
		 * Removes an event handler from this element (shorthand for
		 * {@link #removeListener}).
		 * 
		 * @param {String}
	 ...
	 * @license <a href="http://www.gnu.org/licenses/gpl.html">GPL 3.0</a>
	 * @constructor
	 * @desc Abstract class. This class should not be instantiated.
	 */
	...
	Ext.ux.ManagedIFrame.ComponentAdapter.prototype = {
		...
		/**
		 * @cfg {String} defaultSrc the default src property assigned to the
		 *      Managed Frame when the component is rendered.
		 * @default null
		 */
		defaultSrc : null,

		/**
		 * @cfg {String} unsupportedText Text to display when the
		 *      IFRAMES/FRAMESETS are disabled by the browser.
		 * 
		 */
		...
		/**
		 * @cfg {Boolean} eventsFollowFrameLinks set true to propagate domready
		 *      and documentloaded events anytime the IFRAME's URL changes
		 * @default true
		 */
		eventsFollowFrameLinks : true,
		...
		/**
		 * @cfg {object} frameConfig Frames DOM configuration options This
		 *      optional configuration permits override of the IFRAME's DOM
		 *      attributes
		 *      @example
		 *      frameConfig : {
		      name : 'framePreview',
		...
		/**
		 * @cfg focusOnLoad True to set focus on the frame Window as soon as its
		 *      document reports loaded. (Many external sites use IE's
		 *      document.createRange to create DOM elements, but to be
		 *      successfull IE requires that the FRAME have focus before the
		 *      method is called)
		...
		/**
		 * @property {Object} frameEl An {@link #Ext.ux.ManagedIFrame.Element}
		 *           reference to rendered frame Element.
		 */
		frameEl : null,

		/**
		 * @cfg {Boolean} useShim True to use to create a transparent shimming
		 *      agent for use in masking the frame during drag operations.
		 * @default false
		 */
		useShim : false,

		/**
		 * @cfg {Boolean} autoScroll True to use overflow:'auto' on the frame
		 *      element and show scroll bars automatically when necessary, false
		 *      to clip any overflowing content (defaults to true).
		 * @default true
		 */
		autoScroll : true,

		/**
		 * @cfg {String/Object} autoLoad Loads this Components frame after the
		 *      Component is rendered with content returned from an XHR call or
		 *      optionally from a form submission. See
		 *      {@link #Ext.ux.ManagedIFrame.ComponentAdapter-load} and
		 *      {@link #Ext.ux.ManagedIFrame.ComponentAdapter-submitAsTarget}
		 *      methods for available configuration options.
		 * @default null
		 */
		autoLoad : null,
		 ...
		 * 
		 * @param {Boolean}
		 *            auto True to set overflow:auto on the frame, false for
		 *            overflow:hidden
		...
		/**
		 * Reset the embedded frame to a neutral domain state and clear its
		 * contents
		 * 
		 * @param {String}src
		 *            (Optional) A specific reset string (eg. 'about:blank') to
		 *            use for resetting the frame.
		 * @param {Function}
		 *            callback (Optional) A callback function invoked when the
		 *            frame reset is complete.
		 * @param {Object}
		...
		/**
		 * Loads the Components frame with the response from a form submit to
		 * the specified URL with the ManagedIframe.Element as it's submit
		 * target.
		 * 
		 * @param {Object}
		 *            submitCfg A config object containing any of the following
		...
		/**
		 * Loads this Components's frame immediately with content returned from
		 * an XHR call.
		 * 
		 * @param {Object/String/Function}
		 *            loadCfg A config object containing any of the following
		 *            options:
		 * 
		 * <pre><code>
		 *      mifPanel.load({
		 ...
		 * </code></pre>
		 * 
		 * The only required property is url. The optional properties nocache,
		 * text and scripts are shorthand for disableCaching, indicatorText and
		 * loadScripts and are used to set their associated property on this
		 * panel Updater instance.
		 * @return {Ext.ux.ManagedIFrame.Component} this
		 */
		load : function(loadCfg) {
		...
		/**
		 * Get the {@link #Ext.ux.ManagedIFrame.Updater} for this panel's
		 * iframe. Enables Ajax-based document replacement of this panel's
		 * iframe document.
		 * 
		 * @return {Ext.ux.ManagedIFrame.Updater} The Updater
		 */
		...
		/**
		 * Sets the embedded Iframe location using its replace method. Note:
		 * invoke the function with no arguments to refresh the iframe based on
		 * the current src value.
		 * 
		 * @param {String/Function}
		 *            url (Optional) A string or reference to a Function that
		 *            returns a URI string when called
		...
		setMIFEvents : function() {
			 ...
			 * @param {Ext.ux.ManagedIFrame.Element}
			 *            frameEl
			 */
			'documentloaded',

			/**
			 * Fires ONLY when an iFrame's Document(DOM) has reach a state where
			 * the DOM may be manipulated (ie same domain policy) Note: This
			 * event is only available when overwriting the iframe document
			 * using the update method and to pages retrieved from a "same
			 * domain". Returning false from the eventHandler stops further
			 * event (documentloaded) processing.
			 * 
			 * @event domready
			 * @memberOf Ext.ux.ManagedIFrame.ComponentAdapter
			 * @param {Ext.ux.ManagedIFrame.Element}
			 ...
			 *            this.frameEl
			 * @param {object}
			 *            message (members: type: {string} literal "message",
			 *            data {Mixed} [the message payload], domain [the
			 *            document domain from which the message originated ],
			 *            uri {string} the document URI of the message sender
			 *            source (Object) the window context of the message
			 *            sender tag {string} optional reference tag sent by the
			 *            message sender
			 *            <p>
			 *            Alternate event handler syntax for message:tag
			 *            filtering Fires upon receipt of a message generated by
			 *            window.sendMessage method which includes a specific
			 *            tag value of the embedded Iframe.window object
			 * 
			 */
			'message',
			 ...
			 *            frameEl
			 * @param {Ext.Event}
			 *            e Note: This event is only available when overwriting
			 *            the iframe document using the update method and to
			 *            pages retrieved from a "same domain". Returning false
			 *            from the eventHandler [MAY] NOT cancel the event, as
			 *            this event is NOT ALWAYS cancellable in all browsers.
			 */
			'blur',

			/**
			 * Fires when the frame gets focus. Note: This event is only
			 * available when overwriting the iframe document using the update
			 * method and to pages retrieved from a "same domain". Returning
			 * false from the eventHandler [MAY] NOT cancel the event, as this
			 * event is NOT ALWAYS cancellable in all browsers.
			 * 
			 * @event focus
			 * @memberOf Ext.ux.ManagedIFrame.ComponentAdapter
			 * @param {Ext.ux.ManagedIFrame.Element}
			 ...
			 * document using the update method and to pages retrieved from a
			 * "same-origin" domain. To prevent numerous scroll events from
			 * being raised use the <i>buffer</i> listener option to limit the
			 * number of times the event is raised.
			 * 
			 * @event scroll
			 * @param {Ext.ux.MIF.Element}
			 *            this.
			...
			/**
			 * Fires when the frames window is resized. Note: This event is only
			 * available when overwriting the iframe document using the update
			 * method and to pages retrieved from a "same domain".
			 * 
			 * @event resize
			 * @memberOf Ext.ux.ManagedIFrame.ComponentAdapter
			 * @param {Ext.ux.ManagedIFrame.Element}
			 ...
			 *            e
			 * @param {Object}
			 *            documentSize A height/width object signifying the new
			 *            document size
			 * @param {Object}
			 *            viewPortSize A height/width object signifying the size
			 *            of the frame's viewport
			 * @param {Object}
			 *            viewSize A height/width object signifying the size of
			 *            the frame's view
			 * 
			 */
			'resize',
			 ...
			 * Note: This event is only available when overwriting the iframe
			 * document using the update method and to pages retrieved from a
			 * "same-origin" domain. Note: Opera does not raise this event.
			 * 
			 * @event unload
			 * @memberOf Ext.ux.ManagedIFrame.ComponentAdapter
			 * @param {Ext.ux.ManagedIFrame.Element}
			...
			/**
			 * Fires when the iFrame has been reset to a neutral domain state
			 * (blank document).
			 * 
			 * @event reset
			 * @param {Ext.ux.ManagedIFrame.Element}
		...
		// Suspend (and queue) host container events until the child
		// MIF.Component is rendered.
		onAdd : function(C) {
	...
	Ext.ux.ManagedIFrame.Component = Ext.extend(Ext.BoxComponent, {
		...
		onRender : function(ct, position) {
			...
			// default child frame's name to that of MIF-parent id (if not
			// specified on frameCfg).
			...
			if (F = this.frameEl = ( this.el ? new MIF.Element(this.el.dom, true) : null )) {
				...
				F.addClass('ux-mif');
				if (this.loadMask) {
					// resolve possible maskEl by Element name eg. 'body',
					// 'bwrap', 'actionEl'
		...
		afterRender : function(container) {
			 ...
			 * border layout. Setup event handlers on the SplitBars and region
			 * panels to enable the frame shims when needed
			 */
			if (F = this.frameEl) {
				...
				/*
				 * Create an img shim if the component participates in a layout
				 * or forced
				 */
				...
				if (this.autoload) {
					...
					if (this.defaultSrc) {
						...
						/*
						 * If this is a no-action frame, reset it first, then
						 * resume parent events allowing access to a fully reset
						 * frame by upstream afterrender/layout events
						 */
	...
	/**
	 * @private this function renders a child MIF.Component to MIF.Panel and
	 *          MIF.Window designed to be called by the constructor of
	 *          higher-level MIF.Components only.
	 */
	function embed_MIF(config) {
		...
		config.items = {
			...
			relayTarget : this
			// direct relay of events to the parent component
		};
		delete config.html;
		delete config.data;
		this.setMIFEvents();
	 ...
	 * @param {String/Object}
	 *            el The element to bind the Updater instance to.
	 */
	Ext.ux.ManagedIFrame.Updater = Ext.extend(Ext.Updater, {

		/**
		 * Display the element's "loading" state. By default, the element is
		 * updated with {@link #indicatorText}. This method may be overridden
		 * to perform a custom action while this Updater is actively updating
		 * its contents.
		 */
	...
	/**
	 * @class Ext.ux.ManagedIFrame.CSS Stylesheet interface object
	 * @version 2.1.4
	 * @author Doug Hendricks. doug[always-At]theactivegroup.com
	 * @donate <a target="tag_donate" href="http://donate.theactivegroup.com"><img
	 *         border="0"
	...
	Ext.ux.ManagedIFrame.CSS = function(hostDocument) {
		...
		if (hostDocument) {
			...
			return {
				 ...
				 * @param {String}
				 *            id An (optional) id to add to the stylesheet for
				 *            later removal
				 * @return {StyleSheet}
				 */
				...
				hashRule : function(rule, sheet, mediaOverride) {
					...
					rule.selectorText && Ext.each( ( mediaSelector || '' ).split(','), function(media) {
				 ...
				 * @private
				 * @param {Object/Array}
				 *            rule CSS Rule (or array of Rules/sheets) to
				 ...
				 *            styles dynamically
				 * @param {String}
				 *            mediaSelector Name of optional CSS media context
				 *            (eg. print, screen)
				 ...
				 * @param {String}
				 *            mediaSelector Name(s) of optional media contexts.
				 *            Multiple may be specified, delimited by commas
				 *            (eg. print,screen)
	...
	Ext.ux.ManagedIFrame.Manager = function() {
		var frames = {};
		var implementation = {
			// private DOMFrameContentLoaded handler for browsers (Gecko,
			// Webkit, Opera) that support it.
			_DOMFrameReadyHandler : function(e) {
				...
				} // nested iframes will throw when accessing target.id
			},
			/**
			 * @cfg {String} shimCls
			 * @default "ux-mif-shim" The default CSS rule applied to MIF image
			 *          shims to toggle their visibility.
			 */
			shimCls : 'ux-mif-shim',
			...
			/**
			 * Shim ALL MIFs (eg. when a region-layout.splitter is on the move
			 * or before start of a drag operation)
			 * 
			 * @methodOf Ext.ux.MIF.Manager
			 */
			showShims : function() {
	 ...
	 *            message
	 * @param {Mixed}
	 *            arg optional argument to include in Error object.
	 */
