...
Ext.onReady(function() {
	var a = Ext.util.CSS;
	if (a) {
		a.getRule(".x-hide-nosize") || a.createStyleSheet(".x-hide-nosize{height:0px!important;width:0px!important;border:none!important;zoom:1;}.x-hide-nosize * {height:0px!important;width:0px!important;border:none!important;zoom:1;}");
		a.refreshCache()
	}
});
( function() {
	...
	e.El = {
		setDisplayed : function(k) {
			var j = this;
			j.visibilityCls ? ( j[k !== false ? "removeClass" : "addClass"](j.visibilityCls) ) : a.setDisplayed.call(j, k);
			return j
		},
		...
		isVisible : function(k) {
			var l = this.visible || ( !this.isStyle(f, c) && ( this.visibilityCls ? !this.hasClass(this.visibilityCls) : !this.isStyle(d, i) ) );
			if (k !== true || !l) {
				return l
			}
	...
	Ext.ux.plugin.VisibilityMode = function(k) {
		Ext.apply(this, k || {});
		var j = Ext.util.CSS;
		if (j && !Ext.isIE && this.fixMaximizedWindow !== false && !Ext.ux.plugin.VisibilityMode.MaxWinFixed) {
			...
			n.on("render", function() {
				if (m.bubble !== false && this.ownerCt) {
					j.call(this.ownerCt, function() {
						this.visFixed || this.on("afterlayout", l, this, {
							single : true
						})
					})
				}
...
( function() {
	...
	var T = u.resolveDocumentCache = function(E, aF) {
		Ext._documents[Ext.id(document, "_doc")] = Ext.elCache;
		var aE = e(E), aG = Ext.isDocument(aE) ? Ext.id(aE) : aF, A = Ext._documents[aG] || null;
	...
	Ext.removeNode = function(aH) {
		...
		if (aG && ( aF = A[aG.id] ) && ( aE = aF.el )) {
			if (aE.dom) {
				Ext.enableNestedListenerRemoval ? m.purgeElement(aE.dom, true) : m.removeAll(aE.dom)
			}
			delete A[aG.id];
			delete aE.dom;
			delete aE._context;
	...
	var f = function(aI, aG) {
		...
		if (!aE) {
			...
			aH = function aF() {
				...
				return aK && aK != arguments.callee ? aK.apply(this, arguments) : undefined
			}
		}
		var aJ = [].concat(aG);
	...
	Ext.applyIf(Ext, {
		...
		isDocument : function(E, A) {
			...
			var aF = aG && ( ( aA.apply(aG) == q ) || ( aG && aG.nodeType == 9 ) );
			if (aF && A) {
				try {
					aF = !!aG.location
				} catch (aE) {
		...
		isWindow : function(A) {
			...
			return E ? !!E.navigator || aA.apply(E) == "[object Window]" : false
		},
		...
		isEvent : function(A) {
			return aA.apply(A) == "[object Event]" || ( Ext.isObject(A) && !Ext.type(o.constructor) && ( window.event && A.clientX && A.clientX == window.event.clientX ) )
		},
		...
		isEventSupported : function(aF, aG) {
			var aE = {
				...
				return ( aI ? ( Ext.isElement(aI) || Ext.isDocument(aI) ? aI.nodeName.toLowerCase() : aJ.self ? "#window" : aJ || "#object" ) : aJ || "div" ) + ":" + aK
			};
			return function(aM, aO) {
				...
				aN = Ext.isString(aI) ? an.createElement(aI) : aO;
				aL = ( !!aN && ( aJ in aN ) );
				aL || ( aL = window.Event && !! ( String(aM).toUpperCase() in window.Event ) );
				if (!aL && aN) {
					aN.setAttribute && aN.setAttribute(aJ, "return;");
					...
					if (aF instanceof J) {
						...
						if (aF.tagName || Ext.isWindow(aF)) {
							...
							if (A[E] && ( aI = A[E].el )) {
								...
								aI = J.addToCache(new ( ao(aF) )(aF, null, aK), null, A)
							}
							return aI
						} else {
							if (aF.isComposite) {
	...
	var ar = {}, L = /(-[a-z])/gi, G = function(A, E) {
		...
		t : Q + p,
		b : Q + au
	}, I = J.data, aB = Ext.getDom, s = Ext.get, af = Ext.DomHelper, aa = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/, aC = Ext.util.CSS, ak = function(E) {
		var A = I(E, M);
		if (A === undefined) {
			I(E, M, A = "")
		}
	...
	J.addMethods({
		...
		insertBefore : function(A, E) {
			( A = aB(A, false, E || this.getDocument()) ).parentNode.insertBefore(this.dom, A);
			return this
		},
		insertAfter : function(A, E) {
			( A = aB(A, false, E || this.getDocument()) ).parentNode.insertBefore(this.dom, A.nextSibling);
			return this
		},
		...
		replaceWith : function(A, aE) {
			...
			E.id = Ext.id(E.dom = A);
			J.addToCache(E.isFlyweight ? new ( ao(E.dom) )(E.dom, null, aF) : E);
			return E
		},
		...
		isVisible : function(A) {
			...
			if (typeof aF != "boolean") {
				aF = !E.hasClass(E.visibilityCls || J.visibilityCls) && !E.isStyle(X, ae) && !E.isStyle(K, Z);
				I(aG, W, aF)
			}
			if (A !== true || !aF) {
				return aF
		...
		setVisible : function(aG, A) {
			...
			if (!A || !aE.anim) {
				if (E == J.ASCLASS) {
					aE[aG ? "removeClass" : "addClass"](aE.visibilityCls || J.visibilityCls)
				} else {
					if (E == J.DISPLAY) {
				...
				}, aE.preanim(arguments, 1), null, 0.35, "easeIn", function() {
		...
		scrollIntoView : function(aE, aH) {
			var aM = this.getDocument(), aN = Ext.getDom(aE, null, aM) || Ext.getBody(aM).dom, aG = this.dom, aF = this.getOffsetsTo(aN), aJ = aF[0] + aN.scrollLeft, aQ = aF[1] + aN.scrollTop, aO = aQ + aG.offsetHeight, E = aJ + aG.offsetWidth, A = aN.clientHeight, aK = parseInt(aN.scrollTop, 10), aP = parseInt(aN.scrollLeft, 10), aI = aK + A, aL = aP + aN.clientWidth;
		...
		contains : function(A) {
			try {
				return !A ? false : u.isAncestor(this.dom, A.dom ? A.dom : A)
			} catch (E) {
				return false
			}
		},
		...
		getStyle : function() {
			var A = at && at.getComputedStyle ? function E(aL) {
				...
				aF = ( aH ) ? aH[aL] : null;
				if (aK) {
					if (aF && ay.test(aL) && aG.position != "absolute" && aF != "0px") {
						aJ = aG.display;
				...
				aJ = aq(aJ);
				return ( ( aG = aH.currentStyle ) ? aG[aJ] : null ) || aH.style[aJ]
			};
			var E = null;
			return A
		}(),
		setStyle : function(aF, aE) {
			...
			for (E in aF) {
				if (aF.hasOwnProperty(E)) {
					aE = aF[E];
					E == Y ? this.setOpacity(aE) : this.dom.style[aq(E)] = aE
				}
			}
			return this
		},
		...
		mask : function(E, aH) {
			...
			if (Ext.isIE && ! ( Ext.isIE7 && Ext.isStrict ) && aJ.getStyle("height") == "auto") {
				aK.setSize(undefined, aJ.getHeight())
			}
			return aK
		},
		unmask : function() {
			var aE = this, aF = aE.dom, A = I(aF, "mask"), E = I(aF, "maskMsg");
			if (A) {
				if (E) {
					E.remove();
		...
		getAnchorXY : function(aG, aL, aQ) {
			...
			aQ = aQ || {};
			var aK = this, aN = this.getDocument(), E = aK.dom == aN.body || aK.dom == aN, aO = aQ.width || E ? u.getViewWidth(false, aN) : aK.getWidth(), aI = aQ.height || E ? u.getViewHeight(false, aN) : aK.getHeight(), aP, A = Math.round, aE = aK.getXY(), aM = aK.getScroll(), aJ = E ? aM.left : !aL ? aE[0] : 0, aH = E ? aM.top : !aL ? aE[1] : 0, aF = {
				c : [A(aO * 0.5), A(aI * 0.5)],
				t : [A(aO * 0.5), 0],
				l : [0, A(aI * 0.5)],
				r : [aO, A(aI * 0.5)],
		...
		getAlignToXY : function(aF, aR, aS) {
			...
			aS = aS || [0, 0];
			aR = ( aR == "?" ? "tl-bl?" : ( !/-/.test(aR) && aR != "" ? "tl-" + aR : aR || "tl-bl" ) ).toLowerCase();
			var a3 = this, aY = a3.dom, a5, a4, aK, aJ, aM, aW, aP, aN = u.getViewWidth(false, a6) - 10, aX = u.getViewHeight(false, a6) - 10, E, aG, aH, aI, aO, aQ, a0 = a6.documentElement, aL = a6.body, aV = ( a0.scrollLeft || aL.scrollLeft || 0 ) + 5, aU = ( a0.scrollTop || aL.scrollTop || 0 ) + 5, aZ = false, aE = "", A = "", aT = aR.match(/^([a-z]+)-([a-z]+)(\?)?$/);
			if (!aT) {
				throw "Element.getAlignToXY with an invalid alignment " + aR
			}
			aE = aT[1];
			A = aT[2];
			aZ = !!aT[3];
		...
		adjustForConstraints : function(aE, A, E) {
			return this.getConstrainToXY(A || this.getDocument(), false, E, aE) || aE
		},
		getConstrainToXY : function(E, A, aE, aG) {
			...
			return function(aI, aQ, aJ, aH) {
				...
				aR -= aJ.right;
				aK -= aJ.bottom;
				var aU = aP + aR, aM = aN + aK, aT = aH || ( !aQ ? this.getXY() : [this.getLeft(true), this.getTop(true)] );
				x = aT[0], y = aT[1], offset = this.getConstrainOffset(), w = this.dom.offsetWidth + offset, h = this.dom.offsetHeight + offset;
				var aO = false;
				if ( ( x + w ) > aU) {
					x = aU - w;
					aO = true
		...
		findParent : function(aJ, aI, E) {
			...
			while (aG && aG.nodeType == 1 && aH < aI && aG != A && aG != aE) {
	...
	Ext.apply(u, {
		...
		getViewWidth : Ext.overload([u.getViewWidth || function(A) {
		}, function() {
			return this.getViewWidth(false)
		}, function(A, E) {
			return A ? this.getDocumentWidth(E) : this.getViewportWidth(E)
		}]),
		getViewHeight : Ext.overload([u.getViewHeight || function(A) {
		}, function() {
			return this.getViewHeight(false)
		}, function(A, E) {
			return A ? this.getDocumentHeight(E) : this.getViewportHeight(E)
		}]),
		getDocumentHeight : Ext.overload([u.getDocumentHeight || t, function(A) {
			if (A = this.getDocument(A)) {
				return Math.max(!this.docIsStrict(A) ? A.body.scrollHeight : A.documentElement.scrollHeight, this.getViewportHeight(A))
			}
			return undefined
		}]),
		getDocumentWidth : Ext.overload([u.getDocumentWidth || t, function(A) {
			if (A = this.getDocument(A)) {
				return Math.max(!this.docIsStrict(A) ? A.body.scrollWidth : A.documentElement.scrollWidth, this.getViewportWidth(A))
			}
			return undefined
		}]),
		getViewportHeight : Ext.overload([u.getViewportHeight || t, function(A) {
			if (A = this.getDocument(A)) {
				if (Ext.isIE) {
					return this.docIsStrict(A) ? A.documentElement.clientHeight : A.body.clientHeight
				} else {
		...
		getViewportWidth : Ext.overload([u.getViewportWidth || t, function(A) {
			if (A = this.getDocument(A)) {
				return !this.docIsStrict(A) && !Ext.isOpera ? A.body.clientWidth : Ext.isIE ? A.documentElement.clientWidth : A.defaultView.innerWidth
			}
			return undefined
		}]),
		getXY : Ext.overload([u.getXY || t, function(A, aF) {
			if (typeof A == "string") {
				A = Ext.getDom(A, null, aF);
				var aE = this.getDocument(A), E = aE ? ( aE.body || aE.documentElement ) : null;
				if (!A || !E || A == E) {
					return [0, 0]
				}
			}
	...
	function U(aH, aG, A, aJ, aK) {
		...
		aK = aK || E.scope;
		if (!aE) {
			throw 'Error listening for "' + aG + '". Element "' + aH + "\" doesn't exist."
		}
		function aI(aM) {
			if (!window.Ext) {
				return
	...
	Ext.apply(m, {
		...
		removeListener : m.un = function(aH, aI, aM, aP) {
			...
			for (aG = 0, aJ = aK.length; aG < aJ; aG++) {
				if (Ext.isArray(aL = aK[aG]) && aL[0] == aM && ( !aP || aL[2] == aP )) {
					aL[4] && aL[4].cancel();
					...
					if (A && aI == "mousewheel" && E.addEventListener) {
		...
		removeAll : function(aE) {
			...
			var E = aE.id, aM = T(aE) || {}, aN = aM[E] || {}, aL = aN.events || {}, aI, aH, aJ, aF, aK, aG, A;
			for (aF in aL) {
				if (aL.hasOwnProperty(aF)) {
					...
					for (aH = 0, aJ = aI.length; aH < aJ; aH++) {
						aK = aI[aH];
						aK[4] && aK[4].cancel();
						if (aK[0] && aK[0].tasks && ( aG = aK[0].tasks.length )) {
							while (aG--) {
						...
						A = aK[1];
						aj.un(aE, aF, aj.extAdapter ? aK[3] : A);
						if (A && aE.addEventListener && aF == "mousewheel") {
							aE.removeEventListener("DOMMouseScroll", A, false)
						}
						if (A && ( an == aE ) && aF == "mousedown") {
							Ext.EventManager.stoppedMouseDownEvent.removeListener(A)
						}
					}
				}
		...
		purgeElement : function(aE, A, aG) {
			aE = Ext.getDom(aE);
			var E = Ext.id(aE), aJ = T(aE), aK = ( aJ[E] || {} ).events || {}, aF, aI, aH;
			if (aG) {
...
( function() {
	...
	Ext.ux.ManagedIFrame.Element = Ext.extend(Ext.Element, {
		constructor : function(element, forceNew, doc) {
			...
			MIM.register(this);
			if (!this._observable) {
				( this._observable = new Ext.util.Observable() ).addEvents("documentloaded", "domready", "exception", "resize", "message", "blur", "focus", "unload", "scroll", "reset");
				this._observable.addEvents("_docready", "_docload")
			}
			this.on(Ext.isIE ? "readystatechange" : "load", this.loadHandler, this, Ext.isOpera ? {
	...
	( function() {
		...
		checkDOM : function(win) {
			...
			var n = 0, frame = this, domReady = false, b, l, d, max = this.domReadyRetries || 2500, polling = false, startLocation = ( this.getFrameDocument() || {
				location : {}
			} ).location.href;
	( function() {
		...
		afterRender : function(container) {
			...
			if (F = this.frameEl) {
				...
				if (this.autoload) {
					...
					if (this.tpl && ( this.frameData || this.data )) {
