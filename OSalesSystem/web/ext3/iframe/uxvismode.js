 ...
 *          Commercial Developer License (CDL) is available at
 *          http://licensing.theactivegroup.com.
 * @singleton
 * @static
 * @desc This plugin provides an alternate mechanism for hiding Ext.Elements and
 *       a new hideMode for Ext.Components.<br />
 *       <p>
 *       It is generally designed for use with all browsers <b>except</b>
 *       Internet Explorer, but may used on that Browser as well.
 *       <p>
 *       If included in a Component as a plugin, it sets it's hideMode to
 *       'nosize' and provides a new supported CSS rule that sets the height and
 *       width of an element and all child elements to 0px (rather than
 *       'display:none', which causes DOM reflow to occur and re-initializes
 *       nested OBJECT, EMBED, and IFRAMES elements)
 *       @example
...
Ext.onReady(function() {
	...
	/*
	 * This important rule solves many of the <object/iframe>.reInit issues
	 * encountered when setting display:none on an upstream(parent) element (on
	 * all Browsers except IE). This default rule enables the new Panel:hideMode
	 * 'nosize'. The rule is designed to set height/width to 0 cia CSS if hidden
	 * or collapsed. Additional selectors also hide 'x-panel-body's within
	 * layouts to prevent container and <object, img, iframe> bleed-thru.
	 */
...
( function() {
	...
	fx.El = {
		...
		/**
		 * Returns true if display is not "none" or the visibilityCls has not
		 * been applied
		 * 
		 * @return {Boolean}
		 */
		 ...
		 * visibility, display, and nosize class properties.
		 * 
		 * @param {Boolean}
		...
		isVisible : function(deep) {
			...
			var p = this.dom.parentNode, bodyRE = /^body/i;
	...
	Ext.ux.plugin.VisibilityMode = function(opt) {
		...
		if (CSS && !Ext.isIE && this.fixMaximizedWindow !== false && !Ext.ux.plugin.VisibilityMode.MaxWinFixed) {
			// Prevent overflow:hidden (reflow) transitions when an Ext.Window
			// is maximize.
			CSS.updateRule('.x-window-maximized-ct', 'overflow', '');
			Ext.ux.plugin.VisibilityMode.MaxWinFixed = true; // only updates
																// the CSS Rule
																// once.
		}
	...
	Ext.extend(Ext.ux.plugin.VisibilityMode, Object, {
		...
		/**
		 * @cfg {Boolean} bubble If true, the VisibilityMode fixes are also
		 *      applied to parent Containers which may also impact DOM reflow.
		 * @default true
		 */
		bubble : true,

		/**
		 * @cfg {Boolean} fixMaximizedWindow If not false, the ext-all.css style
		 *      rule 'x-window-maximized-ct' is disabled to <b>prevent</b>
		 *      reflow after overflow:hidden is applied to the document.body.
		 * @default true
		 */
		...
		/**
		 * 
		 * @cfg {array} elements (optional) A list of additional named component
		 *      members to also adjust visibility for. <br />
		 *      By default, the plugin handles most scenarios automatically.
		 * @default null
		 *          @example
		 *          ['bwrap','toptoolbar']
		 */
		...
		/**
		 * @cfg {String} visibilityCls A specific CSS classname to apply to
		 *      Component element when hidden/made visible.
		...
		/**
		 * @cfg {String} hideMode A specific hideMode value to assign to
		 *      affected Components.
		 * @default 'nosize'
		 */
		hideMode : 'nosize',
		 ...
		 * 
		 * @param {Ext.Component}
		 *            c The Ext.Component (or subclass) for which to apply
		 *            visibilityMode treatment
		 */
		init : function(c) {
		...
		/**
		 * @param {Element/Array}
		 *            el The Ext.Element (or Array of Elements) to extend
		 *            visibilityCls handling to.
		 * @param {String}
		 *            visibilityCls The className to apply to the Element when
		 *            hidden.
		 * @return this
		 */
