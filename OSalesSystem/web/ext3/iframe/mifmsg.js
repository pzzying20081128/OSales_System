 ...
 * @version 2.1.2
 * 
 * License: ux.ManagedIFrame, ux.ManagedIFramePanel, ux.ManagedIFrameWindow are
 * licensed under the terms of the Open Source GPL 3.0 license:
...
( function() {
		...
		window.sendMessage = function(message, tag, domain) {
			...
			if (MIF = arguments.callee.manager) {
				if (message._fromHost) {
					...
					try {
						return MIF.disableMessaging !== true ? MIF._observable ? MIF._observable.fireEvent.call(MIF._observable, message.type, MIF, message) : null : null;
					} catch (ex) {
	...
	Ext.override(MIFEl, {
		 ...
		 *            to document.domain).
		 *            <p>
		 *            Notes: on IE8, this action is synchronous.
		 */
	...
	Ext.each(['Component', 'Panel', 'Window'], function(K) {
		MIF[K] && Ext.override(MIF[K], {
			 ...
			 *            tag Optional reference tag
			 * @param {String}
			 *            origin Optional domain designation of the sender
	...
	MM && Ext.apply(MM, {
		...
		destroy : MM.destroy.createSequence(function() {
			( window.postMessage || document.postMessage ) && Ext.EventManager.un(window, 'message', this.onMessage, this);
		})

	});

	// Add support for postMessage message events if the browser supports it
	MM && ( window.postMessage || document.postMessage ) && Ext.EventManager.on(window, 'message', MM.onMessage, MM);
