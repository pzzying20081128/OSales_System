/*
 * ux.ManagedIFrame.* 2.0 RC3 Copyright(c) 2007-2009, Active Group, Inc.
...
( function() {
	...
	var El = Ext.Element, A = Ext.lib.Anim, VISMODE = 'visibilityMode', ELDISPLAY = El.DISPLAY, data = El.data; // >
																												// Ext
																												// 3
																												// RC2
																												// only
	...
	if (!El.ASCLASS) {
		...
		Ext.override(El, {
			...
			getVisibilityMode : function() {
				...
				var dom = this.dom, mode = Ext.type(data) == 'function' ? data(dom, VISMODE) : this[VISMODE];
				if (mode === undefined) {
					mode = 1;
			...
			setVisible : function(visible, animate) {
				var me = this, dom = me.dom, visMode = me.getVisibilityMode();
			...
			isVisible : function(deep) {
				var vis = ! ( this.getStyle("visibility") === "hidden" || this.getStyle("display") === "none" || this.hasClass(this.visibilityCls || El.visibilityCls) );
				if (deep && vis) {
					...
					delete El._flyweights['_isVisible']; // orphan reference
															// cleanup
				}
				return vis;
			}
...
Ext.ux.plugin.VisibilityMode = function(opt) {
	...
	if (CSS && !Ext.isIE && this.fixMaximizedWindow !== false && !Ext.ux.plugin.VisibilityMode.MaxWinFixed) {
		// Prevent overflow:hidden (reflow) transitions when an Ext.Window is
		// maximize.
		CSS.updateRule('.x-window-maximized-ct', 'overflow', '');
		Ext.ux.plugin.VisibilityMode.MaxWinFixed = true; // only updates the
															// CSS Rule once.
	}
...
Ext.extend(Ext.ux.plugin.VisibilityMode, Object, {
	...
	init : function(c) {
		...
		var changeVis = function() {
			...
			this.floating ? null : this.actionMode].concat(plugin.elements || []);
...
( function() {
	...
	var resolveCache = ELD.resolveCache = function(doc, cacheId) {
		...
		if (!cache && doc && ( win = doc.parentWindow || doc.defaultView )) { // Is
																				// it a
																				// frame
																				// document
		...
		return cache || _documents[c] || ( c ? _documents[c] = {
			_elCache : {},
			_dataCache : {}
		} : null );
	};
	...
	Ext.applyIf(Ext, {
		...
		isIterable : function(obj) {
			...
			if (obj === null || obj === undefined)
				return false;
			if (Ext.isArray(obj) || !!obj.callee || Ext.isNumber(obj.length))
				return true;

			return !! ( ( /NodeList|HTMLCollection/i ).test(OP.toString.call(obj)) || // check
																						// for
																						// node
																						// list
																						// type
		...
		isEventSupported : ( function() {
			...
			var getKey = function(type, el) {
				return ( el ? ( Ext.isElement(el) || Ext.isDocument(el, true) ? el.nodeName.toLowerCase() : el.id || Ext.type(el) ) : 'div' ) + ':' + type;
			};
	...
	Ext.apply(Ext, {
		...
		get : Ext.overload([Ext.get, function(el, doc, elcache) { // named-cache
																	// optimized
			try {
			...
			if (typeof el == "string") { // element id
				...
				el.dom = doc.getElementById(el.id) || el.dom; // refresh dom
																// element in
																// case no
																// longer valid,
				// catch case where it hasn't been appended
				el.dom && ( cache[el.id] = el ); // in case it was created
													// directly with Element(),
													// let's cache it
	...
	}, opacityRe = /alpha\(opacity=(.*)\)/i, trimRe = /^\s+|\s+$/g, propFloat = Ext.isIE ? 'styleFloat' : 'cssFloat', view = document.defaultView, VISMODE = 'visibilityMode', ELDISPLAY = El.DISPLAY, data = El.data, CSS = Ext.util.CSS; // Not
																																																											// available
																																																											// in
																																																											// Ext
																																																											// Core.
	...
	El.addMethods({
		...
		getVisibilityMode : function() {
			...
			var dom = this.dom, mode = Ext.isFunction(data) ? data(dom, VISMODE) : this[VISMODE];
			if (mode === undefined) {
				mode = 1;
		...
		scrollIntoView : function(container, hscroll) {
			...
			var el = this.dom;
			var o = this.getOffsetsTo(c), s = this.getScroll(), l = o[0] + s.left, t = o[1] + s.top, b = t + el.offsetHeight, r = l + el.offsetWidth;
		...
		getAnchorXY : function(anchor, local, s) {
			...
			var me = this, doc = this.getDocument(), vp = me.dom == doc.body || me.dom == doc, w = s.width || vp ? Ext.lib.Dom.getViewWidth(false, doc) : me.getWidth(), h = s.height || vp ? Ext.lib.Dom.getViewHeight(false, doc) : me.getHeight(), xy, r = Math.round, o = me.getXY(), scroll = me.getScroll(), extraX = vp ? scroll.left : !local ? o[0] : 0, extraY = vp ? scroll.top : !local ? o[1] : 0, hash = {
				c : [r(w * .5), r(h * .5)],
				t : [r(w * .5), 0],
				l : [0, r(h * .5)],
				r : [w, r(h * .5)],
		...
		getAlignToXY : function(el, p, o) {
			...
			w, h, r, dw = Ext.lib.Dom.getViewWidth(false, doc) - 10, // 10px
																		// of
																		// margin
																		// for
																		// ie
			dh = Ext.lib.Dom.getViewHeight(false, doc) - 10, // 10px of
																// margin for ie
	...
	Ext.apply(ELD, {
		...
		getDocument : function(el, accessTest) {
			var dom = null;
			try {
				dom = Ext.getDom(el, null); // will fail if El.dom is non
		...
		getXY : Ext.overload([ELD.getXY || function(el) {
		}, function(el, doc) {
	...
	Ext.fly = El.fly = Ext.overload([El.fly, // existing 2 arg form
	function(el) {
...
( function() {
	...
	var frameEvents = ['documentloaded', 'domready', 'focus', 'blur', 'resize', 'unload', 'exception', 'message'];
	...
	Ext.ux.ManagedIFrame.Element = Ext.extend(Ext.Element, {
		...
		constructor : function(element, forceNew, doc) {
			...
			this.dom[Ext.isIE ? 'onreadystatechange' : 'onload'] = this.dom['onerror'] = this.loadHandler.createDelegate(this);
		...
		submitAsTarget : function(submitCfg) { // form, url, params, callback,
												// scope){
	...
	// slight delay for masking
		...
		reset : function(src, callback, scope) {
			...
			this._observable.addListener('_docload', function(frame) {
				if (frame.loadMask) {
		...
		_renderHook : function() {
			...
			this._hooked = false;
			try {
				if (this.writeScript('(function(){(window.hostMIF = parent.document.getElementById("' + this.id + '").ownerCt)._windowContext=' + ( Ext.isIE ? 'window' : '{eval:function(s){return eval(s);}}' ) + ';})()')) {
					var w;
					if (w = this.getWindow()) {
		...
		_onDocReady : function(eventName) {
			...
			if (!this._domFired && !this._isReset && ( this._hooked = this._renderHook() )) {
		...
		checkDOM : function(win) {
			if (Ext.isOpera || Ext.isGecko) {
				return;
			}
			// initialise the counter
			var n = 0, manager = this, domReady = false, b, l, d, max = this.domReadyRetries || 2500, // default
																										// max
																										// 5
																										// seconds
		...
		mask : function(msg, msgCls, maskCls) {
			...
			if (typeof msg == 'string') {
				...
				this._maskMsg = Ext.DomHelper.append(p, {
					cls : msgCls || this.cls + "-el-mask-msg",
					style : {
						visibility : 'hidden'
					},
					cn : {
	...
	( function	() {
					this._mask && this._maskMsg && this._maskMsg.center(p).setVisible(true);
				} ).defer(delay, this);
			}
		...
		createShim : function(imgUrl, shimCls) {
			...
			this.shim || ( this.shim = this.next('.' + this.shimCls) || // already
																		// there
																		// ?
	...
	Ext.ux.ManagedIFrame.ComponentAdapter.prototype = {
		...
		setAutoScroll : function(auto) {
			...
			this.rendered && this.getFrame() && this.frameEl.setOverflow(scroll ? 'auto' : 'hidden');
		},
		...
		load : function(loadCfg) {
			this.getFrame() && this.resetFrame(null, this.frameEl.load.createDelegate(this.frameEl, arguments));
			return this;
		},
	...
	Ext.ux.ManagedIFrame.Component = Ext.extend(Ext.BoxComponent, {
		...
		onRender : function() {
			...
			if (F = this.frameEl = ( !!frame ? new MIF.Element(frame, true) : null )) {
				...
				F.setVisibilityMode( ( this.hideMode ? El[this.hideMode.toUpperCase()] : null ) || ElFrame.NOSIZE);
		...
		afterRender : function(container) {
			...
			if (this.frameEl) {
					...
					ownerCt.on('afterlayout', function(container, layout) {
						Ext.each(['north', 'south', 'east', 'west'], function(region) {
							...
							if ( ( reg = layout[region] ) && reg.split && !reg._splitTrapped) {
								...
								reg.panel.on('resize', MIM.hideShims, MIM, {
									delay : 1
								});
				...
				if (!!this.ownerCt || this.useShim) {
					this.shim = this.frameEl.createShim();
				}
		...
		beforeDestroy : function() {
			this.rendered && Ext.each(['frameEl', 'shim'], function(elName) {
				if (this[elName]) {
	...
	Ext.ux.ManagedIFrame.Manager = function() {
		...
		document.addEventListener && window.addEventListener("DOMFrameContentLoaded", implementation._GeckoFrameReadyHandler, true);
