...
Ext.ux.tree.TreeGridNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
	...
	renderElements : function(n, a, targetNode, bulkRender) {
		...
		// (c.tpl ? c.tpl.apply(a) : (typeof(c.renderer)=='undefined' ?
		// a[c.dataIndex]: c.renderer(a[c.dataIndex],a,i) ) || c.text),
		( typeof ( c.renderer ) == 'undefined' ? a[c.dataIndex] : c.renderer(a[c.dataIndex], a, i) ), '</span></a>', '</td>'];

		for (i = 1, len = cols.length; i < len; i++) {
			c = cols[i];
			buf.push('<td class="x-treegrid-col ', ( c.cls ? c.cls : '' ), '">', '<div unselectable="on" class="x-treegrid-text"', ( c.align ? ' style="text-align: ' + c.align + ';"' : '' ), '>',
			// value, cellmeta, record, rowIndex, columnIndex, store
			// (c.tpl ? c.tpl.apply(a) : ( typeof(c.renderer)=='undefined' ?
			// a[c.dataIndex] : c.renderer(a[c.dataIndex],a,i) ) ),
			( typeof ( c.renderer ) == 'undefined' ? a[c.dataIndex] : c.renderer(a[c.dataIndex], a, i) ),
