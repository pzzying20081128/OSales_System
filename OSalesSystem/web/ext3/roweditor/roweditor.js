...
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
	...
	initComponent : function() {
		...
		this.addEvents(
		/**
		 * @event beforeedit Fired before the row editor is activated. If the
		 *        listener returns <tt>false</tt> the editor will not be
		 *        activated.
		 ...
		 *            roweditor This object
		 * @param {Boolean}
		 *            forced True if the cancel button is pressed, false is the
		 *            editor was invalid.
		 */
		'canceledit',
		/**
		 * @event validateedit Fired after a row is edited and passes
		 *        validation. If the listener returns <tt>false</tt> changes
		 *        to the record will not be set.
		 * @param {Ext.ux.grid.RowEditor}
		 *            roweditor This object
		 * @param {Object}
		...
		'validateedit',
		/**
		 * @event afteredit Fired after a row is edited and passes validation.
		 *        This event is fired after the store's update event is fired
		 *        with this edit.
		 * @param {Ext.ux.grid.RowEditor}
		 *            roweditor This object
		 * @param {Object}
	...
	startEditing : function(rowIndex, doFocus) {
		...
		if (this.fireEvent('beforeedit', this, rowIndex) !== false) {
			this.editing = true;
			var g = this.grid, view = g.getView(), row = view.getRow(rowIndex), record = g.store.getAt(rowIndex);
			...
			for (var i = 0, len = cm.getColumnCount(); i < len; i++) {
				...
				val = this.preEditValue(record, cm.getDataIndex(i), column, g.store, rowIndex);
				f = fields[i];
				f.setValue(val);
	...
	initFields : function() {
		...
		for (var i = 0, len = cm.getColumnCount(); i < len; i++) {
			...
			if (!ed) {
				ed = c.displayEditor || new Ext.form.ERPShowTextField();
			}
			if (i == 0) {
	...
	positionButtons : function() {
		if (this.btns) {
			var g = this.grid, h = this.el.dom.clientHeight, view = g.getView(), scroll = view.scroller.dom.scrollLeft, bw = this.btns.getWidth(), width = Math.min(g.getWidth(), g.getColumnModel().getTotalWidth());
	...
	// private
	preEditValue : function(record, field, column, store, rowIndex) {
		// var value = r.data[field];
		// return this.autoEncode && typeof value === 'string' ?
		// Ext.util.Format.htmlDecode(value) : value;
		//
		var value;
		...
		if (field == "" && column.id != null && column.id == "numberer") {
			return column.editor.renderer(params);
		}

		if (field == 'isSelect')
			return null;
		if (column.editor != null && column.editor.renderer != null) {
			return column.editor.renderer(params);
		}
		if (column.renderer != null) {

			return column.renderer.call(column, value, {}, record, store);
		}
		return value;
	...
	doFocus : function(pt) {
		if (this.isVisible()) {
			var index = 0, cm = this.grid.getColumnModel(), c;
			if (pt) {
	...
	getTargetColumnIndex : function(pt) {
		var grid = this.grid, v = grid.view, x = pt.left, cms = grid.colModel.config, i = 0, match = false;
