...
Ext.ux.form.SuperBoxSelect = function(config) {
	 ...
	 * Return false from the callback function to prevent the item from being
	 * added.
	 * 
	 * @event beforeadditem
	...
	/**
	 * Fires when the allowAddNewData config is set to true, and a user attempts
	 * to add an item that is not in the data store.
	 * 
	 * @event newitem
	 * @memberOf Ext.ux.form.SuperBoxSelect
	 * @param {SuperBoxSelect}
...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect, Ext.form.ComboBox, {
	...
	/**
	 * @cfg {Number} itemDelimiterKey The key code which terminates keying in of
	...
	/**
	 * @cfg {Boolean} pinList When set to true the select list will be pinned to
	 *      allow for multiple selections. Defaults to true.
	 */
	pinList : true,
	...
	/**
	 * @cfg {Boolean} supressClearValueRemoveEvents When true, the removeitem
	 *      event will not be fired for each item when the clearValue method is
	 *      called, or when the clear button is used. Defaults to false.
	 */
	supressClearValueRemoveEvents : true, // false
	...
	initComponent : function() {
		...
		if (this.mode === 'remote' && this.store) {
			...
			// this.onStoreLoad(this.store,this.store);
		}
	},
	...
	doTransform : function() {
		...
		if (!this.store) {
			...
			for (var i = 0, len = opts.length; i < len; i++) {
				var o = opts[i], oe = Ext.get(o), value = oe.getAttributeNS(null, 'value') || '', cls = oe.getAttributeNS(null, 'className') || '', style = oe.getAttributeNS(null, 'style') || '';
				if (o.selected) {
		...
		switch (this.msgTarget) {
			...
			case 'under' :
				if (!this.errorEl) {
					...
					this.errorEl.setWidth(elp.getWidth(true) - 35);
				}
	...
	onResize : function(w, h, rw, rh) {
		var reduce = Ext.isIE6 ? 4 : Ext.isIE7 ? 1 : Ext.isIE8 ? 1 : 8;
...
( function			() {
	...
	addRecord : function(record) {
		var display = record.data[this.displayField], caption = this.getCaption(record), val = record.data[this.valueField], cls = this.classField ? record.data[this.classField] : '', style = this.styleField ? record.data[this.styleField] : '';
	...
	setValueEx : function(data) {
		...
		if (this.allowAddNewData && this.mode === 'remote') { // no need to
																// query
