...
( function() {
	var idSeed = 0, toString = Object.prototype.toString, ua = navigator.userAgent.toLowerCase(), check = function(r) {
		return r.test(ua);
	}, DOC = document, docMode = DOC.documentMode, isStrict = DOC.compatMode == "CSS1Compat", isOpera = check(/opera/), isChrome = check(/\bchrome\b/), isWebKit = check(/webkit/), isSafari = !isChrome && check(/safari/), isSafari2 = isSafari && check(/applewebkit\/4/), // unique
																																																																				// to
																																																																				// Safari
																																																																				// 2
...
Ext.apply(Function.prototype, {
	/**
	 * Creates an interceptor function. The passed function is called before the
	 * original one. If it returns false, the original one is not called. The
	 * resulting function returns the results of the original function. The
	 * passed function is called with the parameters of the original function.
	 * Example usage:
	 * 
	 * <pre><code>
	 ...
	 *            reference) in which the passed function is executed. <b>If
	 *            omitted, defaults to the scope in which the original function
	 ...
	 * function that is bound to those 2 args. <b>If a specific scope is
	 * required in the callback, use {@link #createDelegate} instead.</b> The
	 * function returned by createCallback always executes in the window scope.
	 * <p>
	 * This method is required when you want to pass arguments to a callback
	 * function. If no arguments are needed, you can simply pass a reference to
	 * the function as a callback (e.g., callback: myFn). However, if you tried
	 * to pass a function with arguments (e.g., callback: myFn(arg1, arg2)) the
	 * function would simply execute immediately when the code is parsed.
	 * Example usage:
	 * 
	 * <pre><code>
	var sayHi = function(name){
	 ...
	 * <tt>this</tt> variable inside the callback points to obj. Example
	 * usage:
	 * 
	 * <pre><code>
	var sayHi = function(name){
	 ...
	 * @param {Object}
	 *            scope (optional) The scope (<code><b>this</b></code>
	 *            reference) in which the function is executed. <b>If omitted,
	 ...
	 * 
	 * @param {Number}
	 *            millis The number of milliseconds for the setTimeout call (if
...
( function() {
	...
	( function() {
		...
		Ext.lib.Dom = {
			...
			getXY : function(el) {
				var p, pe, b, bt, bl, dbd, x = 0, y = 0, scroll, hasAbsolute, bd = ( doc.body || doc.documentElement ), ret = [0, 0];
