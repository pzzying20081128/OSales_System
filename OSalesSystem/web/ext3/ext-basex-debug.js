 ...
 * Copyright(c) 2008-2009 Active Group, Inc. licensing@theactivegroup.com
 ...
 * options.async =false ) - Permits IE to Access Local File Systems using IE's
 ...
 * to the existing Response Object. - Adds standard HTTP Auth cache support to
 * every request (XHR userId, password config options) - options.method prevails
 * over any method derived by the lib.Ajax stack (DELETE, PUT, HEAD etc). - Adds
 ...
 * Ext.capabilities.isEventSupported('resize'[, forElement]) to determine if the
 * browser supports a specific event. SVG, Canvas, Flash, Cookies, XPath,
 * Audio(HTML5), ChromeFrame (IE) if(Ext.capabilities.hasFlash){ ... } - Adds
 * Ext.overload supported for parameter-based overloading of Function and class
 * methods. - Adds Ext.clone functions for any datatype. - Adds Array prototype
 * features: first, last, clone, forEach, atRandom, include, flatten, compact,
 * unique, filter, map - Connection/response object members :
 * getAllResponseHeaders, getResponseHeader are now functions. - Adds
 * Array.slice support for other browsers (Gecko already supports it)
 * @example: Array.slice( someArray, 2 ) - Adds Ext[isFunction, isObject,
 *           isDocument, isElement, isEvent] methods. - Adds multiPart Response
 *           handling (via onpart callbacks and/or parts Array of response
 *           Object) - Adds parsed contentType to response objects - Adds
 *           Xdomain request support for modern browsers
 * 
 * ***********************************************************************************
...
( function() {
	var A = Ext.lib.Ajax, defined = function(test) {
		return typeof test != 'undefined';
	}, emptyFn = Ext.emptyFn || function() {
	}, OP = Object.prototype, trial = function(fn, scope, args) {
	...
	Ext.extend(Ext.lib.Ajax.Queue, Object, {
		 ...
		 * 
		 * @param {Array}
		 *            request An array of request method arguments.
		 * 
		 */
		add : function(req) {
		...
		/**
		 * @property {Boolean} suspended Indicate the suspense state of the
		 *           queue.
		 */
		suspended : false,
		/**
		 * @property {Object} activeRequest A reference to current/last active
		 *           request.
		 */
		activeRequest : null,
		 ...
		 * 
		 * @param {Boolean}
		 *            peek If true, the queue item is returned but not removed
		 * from the stack. @ default false
		 */
		next : function(peek) {
			var req = peek ? this.requests[this.FIFO ? 'first' : 'last']() : this.requests[this.FIFO ? 'shift' : 'pop']();
		...
		/**
		 * Suspend queue further queue dispatches of any remaining (pending)
		 * requests until the {@link #Ext.ux.ModuleManager-resume} method is
		 * called.
		 */
		suspend : function() {
			this.suspended = true;
		...
		/**
		 * Dispatches the next queue item and initiates a Ext.lib.Ajax request
		 * on the result.
		 * 
		 * @param {Boolean}
		 *            peek If true, the queue item is returned but not removed
	...
	Ext.lib.Ajax.QueueManager = function(config) {
		...
		Ext.apply(this, config || {}, {
			...
			priorityQueues : new Array(new Array(), new Array(), new Array(), new Array(), new Array(), new Array(), new Array(), new Array(), new Array(), new Array()), // iterable
	...
	Ext.extend(Ext.lib.Ajax.QueueManager, Object, {
		/**
		 * @cfg {Integer} quantas Adjustable milliseconds deferred dispatch
		 *      timer interval
		 */
		quantas : 10,

		/**
		 * Return a named queue reference param {String} name The name of the
		 * desired queue.
		 * 
		 ...
		 * 
		 * @param {String/Ext.lib.Ajax.Queue}
		 *            queue
		 */
		...
		/**
		 * @cfg (Boolean) progressive Default Dispatch mode for all defined
		 *      queues
		 *      <p>
		 *      a false value will exhaust a priority queue until empty during
		 *      dispatch (sequential)
		 *      <p>
		 *      true to dispatch a single request from each priority queue until
		...
		dispatch : function() {
			var qm = this, qmq = qm.queues, quit = ( A.activeRequests >= A.maxConcurrentRequests );
	...
	Ext.apply(A, {
		...
		/**
		 * private -- dynamic <script and link> tag support (with event callback
		 * where supported)
		 */
		monitoredNode : function(tag, attributes, callback, context, deferred) {

			var node = null, doc = ( context || window ).document, head = doc ? doc.getElementsByTagName("head")[0] : null, isScript = String(tag).toUpperCase() == 'SCRIPT';
			...
			if (tag && doc && head) {
				...
				var ndom = Ext.getDom(node), canEvent = Ext.capabilities.isEventSupported;

				ndom && forEach(attributes || {}, function(value, attrib) {
					value && ( attrib in ndom ) && ndom.setAttribute(attrib, value);
				});

				var callbacks = Ext.isFunction(callback) ? {
					success : callback
				} : callback || {}, success = Ext.isFunction(callbacks.success) ? callbacks.success.createDelegate(callbacks.scope || callbacks.success, [callbacks], 0) : null, failure = Ext.isFunction(callbacks.failure) ? callbacks.failure.createDelegate(callbacks.scope || callbacks.failure, [callbacks], 0) : null;
		...
		/**
		 * @property maxConcurrentRequests Specify the maximum allowed during
		 *           concurrent Queued browser (XHR) requests to consider during
		 *           queued operation Note: IE8 increases this limit to 6
		 */
		...
		createXhrObject : function(transactionId, options) {
			...
			try {
				...
				if (!defined(obj.conn) && Ext.capabilities.hasActiveX && !!Ext.value(options.forceActiveX, this.forceActiveX)) {
					throw ( "IE7forceActiveX" );
				}
				obj.conn || ( obj.conn = new XMLHttpRequest() );

			} catch (eo) {
				var actX = Ext.capabilities.hasActiveX ? ( options.multiPart ? this.activeXMultipart : this.activeX ) : null;
		...
		serializeForm : function() {
			var reSelect = /select-(one|multiple)/i, reInput = /file|undefined|reset|button/i, reChecks = /radio|checkbox/i, reSubmit = /submit/i;

			return function(form) {
				var fElements = form.elements || ( document.forms[form] || Ext.getDom(form) ).elements, hasSubmit = false, encoder = this.encoder, element, options, name, val, data = '', type, hasValue;
		...
		getHttpStatus : function(reqObj, isAbort, isTimeout) {
			...
			try {
				if (!reqObj || ! ( 'status' in reqObj )) {
					throw ( 'noobj' );
				}
				statObj.status = reqObj.status;
				statObj.readyState = reqObj.readyState;
				statObj.isLocal = ( !reqObj.status && location.protocol == "file:" ) || ( Ext.isSafari && !defined(reqObj.status) );

				statObj.isOK = ( statObj.isLocal || ( statObj.status == 304 || statObj.status == 1223 || ( statObj.status > 199 && statObj.status < 300 ) ) );

				statObj.statusText = reqObj.statusText || '';
			} catch (e) {
			} // status may not avail/valid yet, called too early, or status
		...
		handleTransactionResponse : function(o, callback, isAbort, isTimeout) {
			...
			/*
			 * checked again in case exception was raised - ActiveX was disabled
			 * during XML-DOM creation? And mixin everything the XHR object had
			 * to offer as well
			 */
			o.status.isError && ( responseObject = Ext.apply({}, responseObject || {}, this.createExceptionObject(o.tId, callback.argument, !!isAbort, !!isTimeout, o.status.error)) );
			...
			if (!this.events || this.fireEvent('status:' + o.status.status, o.status.status, o, responseObject, callback, isAbort) !== false) {
				...
				if (o.status.isOK && !o.status.isError) {
					if (!this.events || this.fireEvent('response', o, responseObject, callback, isAbort, isTimeout) !== false) {

						var cb = o.isPart ? 'onpart' : 'success';

						Ext.isFunction(callback[cb]) && callback[cb].call(callback.scope || null, responseObject);

					}
				} else {
					if (!this.events || this.fireEvent('exception', o, responseObject, callback, isAbort, isTimeout, responseObject.fullStatus.error) !== false) {
						Ext.isFunction(callback.failure) && callback.failure.call(callback.scope || null, responseObject, responseObject.fullStatus.error);
		...
		/**
		 * @private Release the allocated XHR object and reset any timers
		 */
		...
		/**
		 * @cfg reCtypeJSON regexp test pattern applied to incoming response
		 *      Content-Type header to identify a potential JSON response. The
		 *      default pattern handles either text/json or application/json
		 */
		reCtypeJSON : /(application|text)\/json/i,

		/**
		 * @cfg reCtypeXML regexp test pattern applied to incoming response
		 *      Content-Type header to identify a potential JSON response. The
		 *      default pattern handles either text/json or application/json
		 */
		...
		createResponseObject : function(o, callbackArg, isAbort, isTimeout) {
			...
			if (isAbort !== true) {
				...
				try {
					...
					headerStr.split('\n').forEach(function(sHeader) {
						( s = sHeader.split(':') ) && s.first() && ( headerObj[s.first().trim().toLowerCase()] = ( s.last() || '' ).trim() );
					});
					...
					obj.contentType = obj.contentType || headerObj[CTYPE] || '';
				}
				...
				if ( ( o.status.isLocal || o.proxied ) && typeof obj.responseText == 'string') {
					...
					if (o.status.isOK && ( ( !obj.responseXML && ( o.options.isXML || this.reCtypeXML.test(obj.contentType) ) ) || ( obj.responseXML && obj.responseXML.childNodes.length === 0 ) )) {
					...
					if (obj.responseXML) {
						var parseBad = ( obj.responseXML.documentElement && obj.responseXML.documentElement.nodeName == 'parsererror' ) || ( obj.responseXML.parseError || 0 ) !== 0 || obj.responseXML.childNodes.length === 0;
						parseBad || ( obj.contentType = headerObj[CTYPE] = obj.responseXML.contentType || 'text\/xml' );
					}
				}

				if (o.options.isJSON || ( this.reCtypeJSON && this.reCtypeJSON.test(headerObj[CTYPE] || "") )) {
					try {
						Ext.isObject(obj.responseJSON) || Ext.isArray(obj.responseJSON) || ( obj.responseJSON = Ext.isFunction(this.decodeJSON) && Ext.isString(obj.responseText) ? this.decodeJSON(obj.responseText) : null );
					} catch (exJSON) {
						o.status.isError = true; // trigger future exception
													// callback
			...
			Ext.apply(obj, {
				...
				getResponseHeader : function(header) {
					return headerObj[ ( header || '' ).trim().toLowerCase()];
				},
		...
		request : function(method, uri, cb, data, options) {
			...
			var _to;
			if (cb.argument && cb.argument.options && cb.argument.options.request && ( _to = cb.argument.options.request.arg )) {
			...
			if (!this.events || this.fireEvent('request', method, uri, cb, data, O) !== false) {

				// Named priority queues
				if (!O.queued && ( O.queue || ( O.queue = this.queueAll || null ) )) {

					O.queue === true && ( O.queue = {
						name : 'q-default'
					} );
					var oq = O.queue;
				...
				options.onpart && ( cb.onpart || ( cb.onpart = Ext.isFunction(options.onpart) ? options.onpart.createDelegate(options.scope) : null ) );

				O.headers && forEach(O.headers, function(value, key) {
					this.initHeader(key, value, false);
				}, this);

				var cType;
				// The Content-Type specified on options.headers always has
				// priority over
				// a calculated value.
				...
				if (O.xmlData) {
					...
					method = 'POST';
					data = ( Ext.isArray(O.jsonData) || Ext.isObject(O.jsonData) ) ? Ext.encode(O.jsonData) : O.jsonData;
				}
				if (data) {
			...
			try {
				if (f = options.proxied) { /* JSONP scriptTag Support */
								...
								if (this.el) {
									...
									if (!o.debug) {
										if (Ext.isIE) {
											...
											// GBG-collect these tags, so help
											// them along
											if (d) {
												for (var prop in d) {
													delete d[prop];
					...
					o.conn.open = function() {
						...
						// apply any new params to any already supplied by the
						// uri and postData
						var params = Ext.urlEncode(Ext.apply(Ext.urlDecode(data) || {}, // decode
																						// any
																						// postData
								o.params, uri.indexOf("?") > -1 ? Ext.urlDecode(uri.split('?').last()) : false));

						o.uri = params ? uri.split('?').first() + '?' + params : uri;

						this.el = A.monitoredNode(f.tag || 'script', {
							type : f.contentType || "text/javascript",
							src : o.uri,
							charset : f.charset || options.charset || null
						},
						/*
						 * If the script tag returns a load signal, evaluate the
						 * response (if any) and set up status for an exception
						 */
						{
							success : function() {
							...
							scope : o.conn
						}, f.target, true); // defer head insertion until send
											// method
		...
		makeRequest : function(method, uri, callback, postData, options) {
			...
			if (o = this.getConnectionObject(uri, options, postData)) {
				...
				try {
					...
					( 'onreadystatechange' in r ) && ( r.onreadystatechange = this.onStateChange.createDelegate(this, [o, callback, 'readystate'], 0) );

					( 'onload' in r ) && ( r.onload = this.onStateChange.createDelegate(this, [o, callback, 'load', 4], 0) );

					( 'onprogress' in r ) && ( r.onprogress = this.onStateChange.createDelegate(this, [o, callback, 'progress'], 0) );
					...
					if (!this.events || this.fireEvent('beforesend', o, method, uri, callback, postData, options) !== false) {
						...
						if (callback && callback.timeout && options.async) {
							...
							( 'ontimeout' in r ) && ( r.ontimeout = this.abort.createDelegate(this, [o, callback, true], 0) );
							( 'ontimeout' in r ) ||
							// Timers for syncro calls won't work here, as it's
							// a blocking call
							( options.async && ( this.timeout[o.tId] = window.setInterval(function() {
								A.abort(o, callback, true);
							}, callback.timeout) ) );
						}

						if (this.useDefaultXhrHeader && !options.xdomain) {
							this.defaultHeaders['X-Requested-With'] || this.initHeader('X-Requested-With', this.defaultXhrHeader, true);
						}
						this.setHeaders(o);
		...
		isCallInProgress : function(o) {
			// if there is a connection and readyState is supported, and not 0
			// or 4
			if (o && o.conn) {
				if ('readyState' in o.conn && {
					0 : true,
					4 : true
				}[o.conn.readyState]) {
					return false;
		 ...
		 * 
		 * @param {String}
		 *            url {optional) reset url for non-IE browsers
		 * @return void
		 */
		...
		/**
		 * @private General readyStateChange multiPart handler
		 */
		onStateChange : function(o, callback, mode) {

			if (!o.conn || o.status.isTimeout || o.status.isError) {
				return;
			}
			...
			if (mode === 'load' || readyState > 2) {
				var ct;
				try {
					ct = C.contentType || C.getResponseHeader('Content-Type') || '';
				} catch (exRs) {
				}
				...
				if (ct && /multipart\//i.test(ct)) {
					...
					if (p) {
						...
						forEach(Array.slice(p, o.parts.length), // skip parts
																// already
																// parsed
						function(newPart) {
							var content = newPart.split('\n\n');
							var H = ( content[0] ? content[0] : '' ) + '\n';
							o.parts.push(this.handleTransactionResponse(Ext.apply(Ext.clone(o), {
								boundary : boundary,
								conn : { // synthetic conn structure for each
											// part
									status : 200,
		...
		setHeaders : function(o) {
			...
			if (o.conn && 'setRequestHeader' in o.conn) {
				this.defaultHeaders && forEach(this.defaultHeaders, function(value, key) {
		...
		// These are only current versions of ActiveX XHR that support multipart
		// responses
		activeXMultipart : ['MSXML2.XMLHTTP.6.0', 'MSXML3.XMLHTTP'],
	...
	if (Ext.util.Observable) {
		...
		Ext.apply(A, {
			...
			/**
			 * onStatus define eventListeners for a single (or array) of HTTP
			 * status codes.
			 */
			...
			/**
			 * unStatus unSet eventListeners for a single (or array) of HTTP
			 * status codes.
			 */
	...
	Ext.applyIf(Array.prototype, {
		...
		map : function(fun, scope) {
			...
			for (var i = 0; i < len; ++i) {
				i in this && ( res[i] = fun.call(scope || this, this[i], i, this) );
			}
			return res;
		},
		...
		include : function(value, deep) { // Boolean: is value present
			...
			try {
				this.forEach(function(item, index) {
					if (found = ( deep ? ( item.include ? item.include(value, deep) : ( item === value ) ) : item === value )) {
		...
		grep : function(rePattern, iterFn, scope) {
			...
			rePattern instanceof RegExp && this.forEach(function(value, index) {
	...
	var overload = function(pfn, fn) {
		...
		if (!ov) {
			...
			f = function() { // the proxy stub
				var o = arguments.callee._ovl, fn = o[arguments.length] || o.base;
				// recursion safety
	...
	}, methodRE = /^(function|object)$/i, primitivesRe = /^(number|string|boolean)$/i;
	...
	Ext.applyIf(Ext, {
		 ...
		 * @argument {String} member The string name of the referenced object's
		 *           member to be tested (Member should be limited to those
		 *           universally implemented as methods)
		 */
		...
		isIterable : function(e) {
			...
			if (/NodeList|HTMLCollection/.test(OP.toString.call(e))) {
				return true;
			}
	 ...
	 * 
	 * @param {String}
	 *            type The eventName (without the 'on' prefix)
	 * @param {HTMLElement/Object/String}
	 *            testEl (optional) A specific HTMLElement/Object to test
	 *            against, otherwise a tagName to test against. based on the
	 *            passed eventName is used, or DIV as default. (window and
	 *            document objects are supported)
	 * @return {Boolean} True if the passed object supports the named event.
	 * @desc Determines whether a specified DOMEvent is supported by a given
	 *       HTMLElement or Object.
	 *       @example
	 *       Does the &lt;script> tag support the load event?
	...
	caps.isEventSupported = function() {
		...
		return function(evName, testEl) {
			...
			var el, isSupported = false, eventName = 'on' + evName, tag = ( testEl ? testEl : TAGNAMES[evName] ) || 'div', key = getKey(evName, tag);
	...
	Ext.onReady(function() {
		 ...
		 * @singleton
		 * @version 4.2
		 * @donate <a target="tag_donate"
		 *         href="http://donate.theactivegroup.com"><img border="0"
		 *         src="http://www.paypal.com/en_US/i/btn/x-click-butcc-donate.gif"
		 *         border="0" alt="Make a donation to support ongoing
		 *         development"></a>
		 * @license <a href="http://www.gnu.org/licenses/gpl.html">GPL 3.0</a>
		...
		Ext.applyIf(caps, {
			/**
			 * @property {Boolean} hasActiveX True if the Browser support (and
			 *           is enabled) ActiveX.
			 */
			hasActiveX : defined(window.ActiveXObject),

			/**
			 * @property {Boolean} hasXDR True if the Browser has native
			...
			/**
			 * @property {Boolean} hasChromeFrame true, if the Google
			 *           ChromeFrame plugin is install on IE
			 */
			hasChromeFrame : function() {
				try {
					if (defined(window.ActiveXObject) && !! ( new ActiveXObject("ChromeTab.ChromeFrame") ))
						return true;
				} catch (ef) {
				}
			...
			/**
			 * @property {Boolean} hasFlash True if the Flash Browser plugin is
			 *           installed.
			 */
			hasFlash : ( function() {
				// Check for ActiveX first because some versions of IE support
				// navigator.plugins, just not the same as other browsers
				if (defined(window.ActiveXObject)) {
					...
					for (var i = 0, P = navigator.plugins, length = P.length; i < length; ++i) {
						// test to see if any plugin names contain the word
						// flash, if so it must support it - return true
				...
				// Return false if ActiveX and nagivator.plugins are not
				// supported
				return false;
			} )(),

			/**
			 * @property {Boolean} hasCookies True if the browser cookies are
			 *           enabled/supported. On IE, ModalDialog windows will
			 *           issue a security risk warning to the user during this
			 *           check, so assert to false. (Cookie implementations on
			 *           IE's [Modeless|Modal]Dialogs are not supported as they
			 *           run in a seperate ActiveX browser context)
			 */
			hasCookies : Ext.isIE && ( 'dialogArguments' in window ) ? false : !!navigator.cookieEnabled,

			/**
			 * @property {Boolean} hasCanvas True if the browser has canvas
			 *           Element support.
			 */
			hasCanvas : !!document.createElement("canvas").getContext,

			/**
			 * @property {Boolean} hasCanvasText True if the browser has canvas
			 *           Element Text support.
			 */
			...
			/**
			 * @property {Boolean} hasXpath True if the browser has Xpath query
			 *           support.
			 */
			hasXpath : !!document.evaluate,

			/**
			 * @property {Boolean} hasWorkers True if the browser has support
			 *           for threaded Workers.
			 */
			hasWorkers : defined(window.Worker) || caps.hasGears,

			/**
			 * @property {Boolean} hasOffline True if the browser has offline
			 *           support.
			 * 
			 */
			...
			/**
			 * @property {Boolean} hasLocalStorage True if the browser has Local
			 *           Storage support. disabling Cookies on Gecko will throw
			 */
			hasLocalStorage : trial(defined, null, window, 'localStorage') || false,

			/**
			 * @property {Boolean} hasDeviceAcceleration True if the browser has
			 *           device acceleration detection abilities.
			 */
			 ...
			 * @desc Basic HTML5 Element support for the &lt;audio> tag and/or
			 *       Audio object.
			 *       @example
			 *       If the browser has &lt;audio> tag or Audio object support,<br />
			 *       the property contains a mime-type map of standard audio
			 *       formats. { mp3 : false, //mp3 ogg : false, //Ogg Vorbis wav :
			 *       true, //wav basic : false, //au, snd aif : false, //aif,
			 *       aifc, aiff tag : true, //is audio HTML element supported?
			 *       object : true, //is the window.Audio Object supported
			 *       <b>testMime</b> : function() }
			 * 
			 * The included <b>testMime</b> function permits selective
			 * mime-type testing as well for custom audio formats:
			 * if(Ext.capabilities.hasAudio &&
			 * Ext.capabilities.hasAudio.testMime('audio/ogg') ){ alert ('Vorbis
			 * playback is supported'); }
			 */
			hasAudio : function() {

				var aTag = !!document.createElement('audio').canPlayType, aAudio = ( 'Audio' in window && window.Audio ) ? new Audio('') : {}, caps = aTag || ( 'canPlayType' in aAudio ) ? {
					tag : aTag,
					...
					testMime : function(mime) {
						var M;
						return ( M = aAudio.canPlayType ? aAudio.canPlayType(mime) : 'no' ) !== 'no' && M !== '';
					}
				} : false, mime, chk, mimes = {
					mp3 : 'audio/mpeg', // mp3
			 ...
			 */
			hasVideo : function() {
				var vTag = !!document.createElement('video').canPlayType, vVideo = vTag ? document.createElement('video') : {}, caps = ( 'canPlayType' in vVideo ) ? {
					tag : vTag,
					/*
					 * Test for a specific video and codec (eg: 'video/ogg;
					 * codecs="theora, vorbis"' )
					 */
					testCodec : function(codec) {
						var C;
						return ( C = vVideo.canPlayType ? vVideo.canPlayType(codec) : 'no' ) !== 'no' && C !== '';
					}
				} : false, codec, chk, codecs = {
					mp4 : 'video/mp4; codecs="avc1.42E01E, mp4a.40.2"', // mp4
																		// (Apple:
																		// patented
																		// licensed
																		// codec)
					ogg : 'video/ogg; codecs="theora, vorbis"' // ogg Vorbis
																// codec
				};
			 ...
			 *       search, number, range, color, tel, url, email, date, month,
			 *       week, tine, datetime, datetime-local
			 */

			hasInputs : function() {
				var i = {};
				Ext.each(['search', 'number', 'range', 'color', 'tel', 'url', 'email', 'date', 'month', 'week', 'time', 'datetime', 'datetime-local'], function(type) {
					try {
	...
	Ext.EventManager.on(window, "beforeunload", A.onUnload, A, {
		single : true
	});
} )();
...
Ext.applyIf(Function.prototype, {
	 ...
	 * Credit:
	 * 
	 * @Animal -- the_bagbournes@btinternet.com Creates a delegate (callback)
	 *         which, when called, executes after a specific delay. Optionally,
	 *         a replacement (or additional) argument list may be specified.
	 * @param {Number}
	 *            delay The number of milliseconds to defer execution by
	 *            whenever called.
	 * @param {Object}
	 *            scope (optional) The scope (<code>this</code> reference)
	 *            used by the function at execution time.
	 * @param {Array}
	 *            args (optional) Override arguments for the call. (Defaults to
	 ...
	 * @return {Function} A function which, when called, executes the original
